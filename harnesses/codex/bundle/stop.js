...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join8(homedir5(), ".claude", "local", cli);
...
import { readFileSync as readFileSync8, writeFileSync as writeFileSync8, writeSync as writeSync2, mkdirSync as mkdirSync10, existsSync as existsSync7, unlinkSync as unlinkSync4, openSync as openSync3, closeSync as closeSync3, statSync as statSync2 } from "node:fs";
import { homedir as homedir10 } from "node:os";
import { join as join16 } from "node:path";
var dlog3 = (msg) => log("summary-state", msg);
var STATE_DIR = join16(homedir10(), ".claude", "hooks", "summary-state");
var YIELD_BUF2 = new Int32Array(new SharedArrayBuffer(4));
function lockPath3(sessionId) {
  return join16(STATE_DIR, `${sessionId}.lock`);
}
...
import { existsSync as existsSync9, mkdirSync as mkdirSync11, readFileSync as readFileSync10, renameSync as renameSync4, writeFileSync as writeFileSync9 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { dirname as dirname6, join as join18 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join18(homedir12(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function defaultResolveTransformers() {
  const sharedDir = join19(homedir13(), ".hivemind", "embed-deps");
  try {
