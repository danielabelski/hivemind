...
import { mkdirSync as mkdirSync10, readFileSync as readFileSync12, writeFileSync as writeFileSync8, renameSync as renameSync6, existsSync as existsSync8 } from "node:fs";
import { homedir as homedir9 } from "node:os";
import { createHash as createHash5 } from "node:crypto";
import { dirname as dirname8, join as join16 } from "node:path";
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join16(homedir9(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash5("sha256").update(`${project}\0${scope}`).digest("hex");
  return join16(privateStoreRoot(), `${key}.json`);
}
function readMap(file) {
  try {
    if (!existsSync8(file))
      return {};
    const raw = JSON.parse(readFileSync12(file, "utf-8"));
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
      return {};
    return raw;
  } catch {
...
function resolveEmbedDaemonPath() {
  return join17(dirname9(fileURLToPath2(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
import { mkdirSync as mkdirSync11, readFileSync as readFileSync13, rmSync, writeFileSync as writeFileSync9 } from "node:fs";
...
function readCachedIndexContent(sessionId, deps = {}) {
  ...
  try {
    return readFileSync13(join18(getSessionQueryCacheDir(sessionId, deps), INDEX_CACHE_FILE), "utf-8");
  } catch (e) {
    if (e?.code === "ENOENT")
      return null;
...
function isDirectRun(metaUrl) {
  ...
  if (!entry)
    return false;
  try {
    return resolve4(fileURLToPath3(metaUrl)) === resolve4(entry);
  } catch {
    return false;
  }
}
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join20(homedir12(), ".deeplake", "state", "skillify");
}
...
import { existsSync as existsSync10, lstatSync, mkdirSync as mkdirSync12, readFileSync as readFileSync14, renameSync as renameSync8, unlinkSync as unlinkSync4, writeFileSync as writeFileSync10 } from "node:fs";
...
function migrateLegacyStateDir() {
  ...
  const legacy = join21(dirname10(current), "skilify");
...
function manifestPath() {
  return join22(getStateDir(), "pulled.json");
}
...
async function processCodexPreToolUse(input, deps = {}) {
  const { config = loadConfig(), createApi = (table, activeConfig) => new DeeplakeApi(activeConfig.token, activeConfig.apiUrl, activeConfig.orgId, activeConfig.workspaceId, table), executeCompiledBashCommandFn = executeCompiledBashCommand, readVirtualPathContentsFn = readVirtualPathContents, readVirtualPathContentFn = readVirtualPathContent, listVirtualPathRowsFn = listVirtualPathRows, findVirtualPathsFn = findVirtualPaths, handleGrepDirectFn = handleGrepDirect, readCachedIndexContentFn = readCachedIndexContent, writeCachedIndexContentFn = writeCachedIndexContent, runVfsShellFn = (command) => {
    const shellBundle = join23(__bundleDir, "shell", "deeplake-shell.js");
