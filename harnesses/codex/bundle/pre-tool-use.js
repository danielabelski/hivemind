...
import { join as join20, dirname as dirname11 } from "node:path";
...
function resolveEmbedDaemonPath() {
  return join14(dirname8(fileURLToPath2(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
var DEFAULT_CACHE_ROOT = join15(homedir9(), ".deeplake", "query-cache");
var INDEX_CACHE_FILE = "index.md";
function getSessionQueryCacheDir(sessionId, deps = {}) {
  const { cacheRoot = DEFAULT_CACHE_ROOT } = deps;
  return join15(cacheRoot, sessionId);
}
function readCachedIndexContent(sessionId, deps = {}) {
  const { logFn = log5 } = deps;
  try {
    return readFileSync10(join15(getSessionQueryCacheDir(sessionId, deps), INDEX_CACHE_FILE), "utf-8");
  } catch (e) {
    if (e?.code === "ENOENT")
      return null;
...
function writeCachedIndexContent(sessionId, content, deps = {}) {
  ...
  try {
    ...
    writeFileSync8(join15(dir, INDEX_CACHE_FILE), content, "utf-8");
  } catch (e) {
...
var MEMORY_PATH = join16(homedir10(), ".deeplake", "memory");
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join17(homedir11(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join18(dirname9(current), "skilify");
...
function manifestPath() {
  return join19(getStateDir(), "pulled.json");
}
