...
import { existsSync as existsSync3, mkdirSync as mkdirSync5, readFileSync as readFileSync6, renameSync as renameSync2, writeFileSync as writeFileSync4 } from "node:fs";
import { dirname as dirname3, join as join7 } from "node:path";
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join7(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join7(baseDir, ".last-build.json");
}
...
import { appendFileSync as appendFileSync2, existsSync as existsSync4, mkdirSync as mkdirSync6, readFileSync as readFileSync7 } from "node:fs";
import { dirname as dirname4, join as join8 } from "node:path";
function historyPath(baseDir) {
  return join8(baseDir, "history.jsonl");
}
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join9(homedir5(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join9(graphsRoot(), repoKey);
}
...
async function pullSnapshot(cwd, deps = {}) {
  ...
  const snapshotPath = join10(snapshotsDir, `${head}.json`);
  const worktreeRoot = join10(baseDir, "worktrees", worktreeId);
  try {
    writeFileAtomic(snapshotPath, cloudPayload);
    writeFileAtomic(join10(worktreeRoot, "latest-commit.txt"), `${head}
`);
...
async function main() {
  ...
  const timeoutPromise = new Promise((resolve5) => {
    setTimeout(() => resolve5("timeout"), workerTimeoutMs()).unref();
  });
  let logLine;
  try {
...
`;
      try {
        ...
        appendFileSync3(join11(dir, ".graph-pull.log"), logLine);
      } catch {
      }
      process.exit(0);
    }
