...
import { existsSync as existsSync8, readFileSync as readFileSync11, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, renameSync as renameSync5, lstatSync as lstatSync2, readlinkSync, symlinkSync, unlinkSync as unlinkSync4 } from "node:fs";
...
import { existsSync as existsSync9, mkdirSync as mkdirSync11, readFileSync as readFileSync12, writeFileSync as writeFileSync9 } from "node:fs";
import { homedir as homedir11 } from "node:os";
import { dirname as dirname8, join as join18 } from "node:path";
var LOCAL_MANIFEST_PATH = join18(homedir11(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join18(homedir11(), ".claude", "hivemind", "local-mined.lock");
...
var HOME = homedir12();
var HIVEMIND_DIR = join19(HOME, ".claude", "hivemind");
var LOG_PATH = join19(HOME, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join19(HOME, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname9(fileURLToPath(import.meta.url));
    const cliPath = join19(thisDir, "..", "..", "bundle", "cli.js");
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync2(join19(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  ...
  try {
    mkdirSync12(join19(HOME, ".claude", "hooks"), { recursive: true });
...
import { existsSync as existsSync11, mkdirSync as mkdirSync13, readFileSync as readFileSync13, renameSync as renameSync6, writeFileSync as writeFileSync10 } from "node:fs";
...
import { appendFileSync as appendFileSync3, existsSync as existsSync12, mkdirSync as mkdirSync14, readFileSync as readFileSync14 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join22(homedir13(), ".hivemind", "graphs");
}
...
function spawnGraphPullWorker(cwd, bundleDir, deps = {}) {
  if (process.env.HIVEMIND_GRAPH_PULL === "0")
    return;
  const workerPath = join24(bundleDir, "graph-pull-worker.js");
  const opts = {
    detached: true,
    stdio: ["ignore", "ignore", "ignore"]
  };
...
import { readFileSync as readFileSync15, writeFileSync as writeFileSync12, writeSync, mkdirSync as mkdirSync16, existsSync as existsSync14, unlinkSync as unlinkSync6, openSync as openSync3, closeSync as closeSync3, statSync as statSync4 } from "node:fs";
import { homedir as homedir14 } from "node:os";
import { join as join25 } from "node:path";
var dlog2 = (msg) => log("summary-state", msg);
var STATE_DIR = join25(homedir14(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath(sessionId) {
  return join25(STATE_DIR, `${sessionId}.json`);
}
function endedMarkerPath(sessionId) {
  return join25(STATE_DIR, `${sessionId}.ended`);
}
function ownerPath(sessionId) {
  return join25(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync15(`/proc/${pid}/stat`, "utf-8");
...
var HOME2 = homedir15();
var { log: wikiLog } = makeWikiLogger(join26(HOME2, ".claude", "hooks"));
