...
import { existsSync as existsSync2, mkdirSync as mkdirSync6, readFileSync as readFileSync6, writeFileSync as writeFileSync5 } from "node:fs";
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync4(dirname2(LOG), { recursive: true });
...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync4, mkdirSync as mkdirSync5, openSync, closeSync, unlinkSync as unlinkSync2, statSync } from "node:fs";
...
function queuePath() {
  return join6(homedir6(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
function writeQueue(q) {
  ...
  mkdirSync5(join6(home, ".deeplake"), { recursive: true, mode: 448 });
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync5(join6(homedir6(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
function getInstalledVersion(bundleDir, pluginManifestDir) {
  ...
  try {
    const stamp = readFileSync7(join8(bundleDir, "..", ".hivemind_version"), "utf-8").trim();
    if (stamp)
      return stamp;
  } catch {
  }
...
import { existsSync as existsSync8, readFileSync as readFileSync10, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, renameSync as renameSync5, lstatSync as lstatSync2, readlinkSync, symlinkSync, unlinkSync as unlinkSync4 } from "node:fs";
import { homedir as homedir10 } from "node:os";
import { dirname as dirname6, join as join16 } from "node:path";

// dist/src/skillify/skill-writer.js
import { existsSync as existsSync4, mkdirSync as mkdirSync8, readFileSync as readFileSync8, readdirSync, statSync as statSync2, writeFileSync as writeFileSync6 } from "node:fs";
...
import { existsSync as existsSync6, lstatSync, mkdirSync as mkdirSync9, readFileSync as readFileSync9, renameSync as renameSync4, unlinkSync as unlinkSync3, writeFileSync as writeFileSync7 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join12(homedir8(), ".deeplake", "state", "skillify");
}
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync6(join14(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync7(join15(home, ".codex"));
  const piInstalled = existsSync7(join15(home, ".pi", "agent"));
  const hermesInstalled = existsSync7(join15(home, ".hermes"));
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join16(homedir10(), ".claude", "skills");
  if (!cwd)
...
import { existsSync as existsSync9, mkdirSync as mkdirSync11, readFileSync as readFileSync11, writeFileSync as writeFileSync9 } from "node:fs";
import { homedir as homedir11 } from "node:os";
import { dirname as dirname7, join as join17 } from "node:path";
var LOCAL_MANIFEST_PATH = join17(homedir11(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join17(homedir11(), ".claude", "hivemind", "local-mined.lock");
...
import { closeSync as closeSync2, existsSync as existsSync10, mkdirSync as mkdirSync12, openSync as openSync2, readdirSync as readdirSync2, statSync as statSync3, unlinkSync as unlinkSync5 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { dirname as dirname8, join as join18 } from "node:path";
import { fileURLToPath } from "node:url";
var HOME = homedir12();
var HIVEMIND_DIR = join18(HOME, ".claude", "hivemind");
var LOG_PATH = join18(HOME, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join18(HOME, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname8(fileURLToPath(import.meta.url));
    const cliPath = join18(thisDir, "..", "..", "bundle", "cli.js");
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync2(join18(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  ...
  try {
    mkdirSync12(join18(HOME, ".claude", "hooks"), { recursive: true });
...
import { existsSync as existsSync11, mkdirSync as mkdirSync13, readFileSync as readFileSync12, renameSync as renameSync6, writeFileSync as writeFileSync10 } from "node:fs";
import { dirname as dirname9, join as join19 } from "node:path";
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join19(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join19(baseDir, ".last-build.json");
}
...
import { mkdirSync as mkdirSync15, renameSync as renameSync7, writeFileSync as writeFileSync11 } from "node:fs";
import { homedir as homedir13 } from "node:os";
import { dirname as dirname11, join as join21 } from "node:path";

// dist/src/graph/history.js
import { appendFileSync as appendFileSync3, existsSync as existsSync12, mkdirSync as mkdirSync14, readFileSync as readFileSync13 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join21(homedir13(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join21(graphsRoot(), repoKey);
}
...
function graphContextLine(cwd, deps = {}) {
  ...
  try {
    ...
    baseDir = repoDir(key);
    snapshotsDir = join22(baseDir, "snapshots");
  } catch {
    return null;
  }
  ...
  const snapshotPath = join22(snapshotsDir, `${snapshotFile}.json`);
...
function spawnGraphPullWorker(cwd, bundleDir, deps = {}) {
  if (process.env.HIVEMIND_GRAPH_PULL === "0")
    return;
  const workerPath = join23(bundleDir, "graph-pull-worker.js");
  const opts = {
    detached: true,
    stdio: ["ignore", "ignore", "ignore"]
  };
...
import { readFileSync as readFileSync14, writeFileSync as writeFileSync12, writeSync, mkdirSync as mkdirSync16, existsSync as existsSync14, unlinkSync as unlinkSync6, openSync as openSync3, closeSync as closeSync3, statSync as statSync4 } from "node:fs";
import { homedir as homedir14 } from "node:os";
import { join as join24 } from "node:path";
var dlog2 = (msg) => log("summary-state", msg);
var STATE_DIR = join24(homedir14(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath(sessionId) {
  return join24(STATE_DIR, `${sessionId}.json`);
}
function endedMarkerPath(sessionId) {
  return join24(STATE_DIR, `${sessionId}.ended`);
}
...
function procInfo(pid) {
  try {
    const s = readFileSync14(`/proc/${pid}/stat`, "utf-8");
...
function recordSessionOwner(sessionId, agentComms = ["claude"], startPid = process.pid) {
  try {
    ...
    writeFileSync12(tmp, JSON.stringify(owner));
    renameAtomic(tmp, p);
  } catch (e) {
...
var HOME2 = homedir15();
var { log: wikiLog } = makeWikiLogger(join25(HOME2, ".claude", "hooks"));
      ...
      if (config) {
        ...
        try {
          ...
          if (maybeSpawnDocsRefresh(cwd, { orgId: config.orgId, project: deriveProjectKey(cwd).key, full: true })) {
            log6("docs auto sync spawned (session-start tick)");
          }
        } catch {
        }
        const known = await api.knownTablesOrNull();
