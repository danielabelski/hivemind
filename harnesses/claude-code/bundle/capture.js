...
import { readFileSync as readFileSync6, writeFileSync as writeFileSync4, writeSync, mkdirSync as mkdirSync5, existsSync as existsSync3, unlinkSync as unlinkSync3, openSync as openSync2, closeSync as closeSync2, statSync as statSync2 } from "node:fs";
...
function statePath(sessionId) {
  return join7(STATE_DIR, `${sessionId}.json`);
}
function lockPath2(sessionId) {
  return join7(STATE_DIR, `${sessionId}.lock`);
}
function ownerPath(sessionId) {
  return join7(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync6(`/proc/${pid}/stat`, "utf-8");
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path2 = join8(hooksDir, filename);
  return {
    path: path2,
    log(msg) {
      try {
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join10(homedir6(), ".claude", "local", cli);
  return isWin ? `${local}.cmd` : local;
}

// dist/src/hooks/spawn-wiki-worker.js
var HOME = homedir7();
var wikiLogger = makeWikiLogger(join11(HOME, ".claude", "hooks"));
...
function spawnWikiWorker(opts) {
  ...
  writeFileSync5(configFile, JSON.stringify({
    ...
    wikiLog: WIKI_LOG,
    hooksDir: join11(HOME, ".claude", "hooks"),
...
import { readFileSync as readFileSync8, writeFileSync as writeFileSync7, writeSync as writeSync2, mkdirSync as mkdirSync9, renameSync as renameSync3, rmdirSync, existsSync as existsSync6, lstatSync, unlinkSync as unlinkSync4, openSync as openSync3, closeSync as closeSync3 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join14(homedir10(), ".deeplake", "state", "skillify");
}
...
import { existsSync as existsSync7, mkdirSync as mkdirSync10, readFileSync as readFileSync9, writeFileSync as writeFileSync8 } from "node:fs";
import { join as join17 } from "node:path";
function configPath() {
  return join17(getStateDir(), "config.json");
}
...
import { existsSync as existsSync8, lstatSync as lstatSync2, mkdirSync as mkdirSync11, readFileSync as readFileSync10, renameSync as renameSync4, unlinkSync as unlinkSync5, writeFileSync as writeFileSync9 } from "node:fs";
...
import { openSync as openSync4, closeSync as closeSync4, writeSync as writeSync3, unlinkSync as unlinkSync6, existsSync as existsSync9, readFileSync as readFileSync11 } from "node:fs";
...
// dist/src/embeddings/client.js
var SHARED_DAEMON_PATH = join19(homedir11(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync11(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync11(this.pidPath, "utf-8").trim();
...
import { existsSync as existsSync10, mkdirSync as mkdirSync12, readFileSync as readFileSync12, renameSync as renameSync5, writeFileSync as writeFileSync10 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { dirname as dirname8, join as join20 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join20(homedir12(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function defaultResolveTransformers() {
  const sharedDir = join21(homedir13(), ".hivemind", "embed-deps");
  try {
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join22(homedir14(), ".claude", "settings.json");
...
import { existsSync as existsSync11, lstatSync as lstatSync3, mkdirSync as mkdirSync13, readlinkSync, renameSync as renameSync6, rmSync, symlinkSync, statSync as statSync3 } from "node:fs";
import { homedir as homedir15 } from "node:os";
import { basename as basename3, dirname as dirname9, join as join23 } from "node:path";
function ensurePluginNodeModulesLink(opts) {
  ...
  const target = opts.sharedNodeModules ?? join23(homedir15(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname9(opts.bundleDir);
  const link = join23(pluginDir, "node_modules");
...
function resolveEmbedDaemonPath() {
  return join24(dirname10(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
