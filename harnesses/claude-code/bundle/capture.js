...
import { appendFileSync as appendFileSync3, mkdirSync as mkdirSync8, readFileSync as readFileSync8, readdirSync, statSync as statSync3, unlinkSync as unlinkSync4 } from "node:fs";
import { homedir as homedir8 } from "node:os";
import { join as join12 } from "node:path";
var dlog2 = (msg) => log("session-event-cache", msg);
var CACHE_DIR = join12(homedir8(), ".claude", "hooks", "session-cache");
...
function sessionEventCachePath(sessionId) {
  return join12(CACHE_DIR, `${sessionId}.jsonl`);
}
...
import { readFileSync as readFileSync9, writeFileSync as writeFileSync7, writeSync as writeSync2, mkdirSync as mkdirSync10, renameSync as renameSync3, rmdirSync, existsSync as existsSync6, lstatSync, unlinkSync as unlinkSync5, openSync as openSync3, closeSync as closeSync3 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join15(homedir11(), ".deeplake", "state", "skillify");
}
...
import { existsSync as existsSync7, mkdirSync as mkdirSync11, readFileSync as readFileSync10, writeFileSync as writeFileSync8 } from "node:fs";
...
import { existsSync as existsSync8, lstatSync as lstatSync2, mkdirSync as mkdirSync12, readFileSync as readFileSync11, renameSync as renameSync4, unlinkSync as unlinkSync6, writeFileSync as writeFileSync9 } from "node:fs";
...
import { openSync as openSync4, closeSync as closeSync4, writeSync as writeSync3, unlinkSync as unlinkSync7, existsSync as existsSync9, readFileSync as readFileSync12 } from "node:fs";
...
// dist/src/embeddings/client.js
var SHARED_DAEMON_PATH = join20(homedir12(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync12(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync12(this.pidPath, "utf-8").trim();
...
import { existsSync as existsSync10, mkdirSync as mkdirSync13, readFileSync as readFileSync13, renameSync as renameSync5, writeFileSync as writeFileSync10 } from "node:fs";
import { homedir as homedir13 } from "node:os";
import { dirname as dirname8, join as join21 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join21(homedir13(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
function defaultResolveTransformers() {
  const sharedDir = join22(homedir14(), ".hivemind", "embed-deps");
  try {
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join23(homedir15(), ".claude", "settings.json");
...
import { basename as basename3, dirname as dirname9, join as join24 } from "node:path";
function ensurePluginNodeModulesLink(opts) {
  ...
  const target = opts.sharedNodeModules ?? join24(homedir16(), ".hivemind", "embed-deps", "node_modules");
  const pluginDir = dirname9(opts.bundleDir);
  const link = join24(pluginDir, "node_modules");
