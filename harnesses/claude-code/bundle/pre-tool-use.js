...
import { join as join21, dirname as dirname11, sep } from "node:path";
...
${firstDocLine(h.content)}`);
  ...
  if (path2 === "" || path2 === "index.md")
    ...
    dir = path2;
  if (dir !== null) {
    const meta = (await listDocMeta(query, tableName, { dirPrefix: dir, project: opts.project })).map((r) => ({
      doc_id: r.doc_id,
      version: r.version,
...
function resolveEmbedDaemonPath() {
  return join17(dirname10(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
var DEFAULT_CACHE_ROOT = join18(homedir10(), ".deeplake", "query-cache");
var INDEX_CACHE_FILE = "index.md";
function getSessionQueryCacheDir(sessionId, deps = {}) {
  const { cacheRoot = DEFAULT_CACHE_ROOT } = deps;
  return join18(cacheRoot, sessionId);
}
function readCachedIndexContent(sessionId, deps = {}) {
  const { logFn = log5 } = deps;
  try {
    return readFileSync11(join18(getSessionQueryCacheDir(sessionId, deps), INDEX_CACHE_FILE), "utf-8");
  } catch (e) {
    if (e?.code === "ENOENT")
      return null;
...
function writeCachedIndexContent(sessionId, content, deps = {}) {
  ...
  try {
    ...
    writeFileSync9(join18(dir, INDEX_CACHE_FILE), content, "utf-8");
  } catch (e) {
...
var MEMORY_PATH = join19(homedir11(), ".deeplake", "memory");
...
import { readFileSync as readFileSync12, writeFileSync as writeFileSync10, writeSync as writeSync2, mkdirSync as mkdirSync12, existsSync as existsSync10, unlinkSync as unlinkSync5, openSync as openSync3, closeSync as closeSync3, statSync as statSync2 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { join as join20 } from "node:path";
var dlog2 = (msg) => log("summary-state", msg);
var STATE_DIR = join20(homedir12(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function ownerPath(sessionId) {
  return join20(STATE_DIR, `${sessionId}.owner`);
}
...
var __bundleDir = dirname11(fileURLToPath5(import.meta.url));
var READ_CACHE_ROOT = join21(homedir13(), ".deeplake", "query-cache");
function writeReadCacheFile(sessionId, virtualPath, content, deps = {}) {
  ...
  const expectedRoot = join21(cacheRoot, safeSessionId, "read");
  const absPath = join21(expectedRoot, rel);
  ...
  mkdirSync13(dirname11(absPath), { recursive: true });
...
async function processPreToolUse(input, deps = {}) {
  ...
  const shellBundle = join21(__bundleDir, "shell", "deeplake-shell.js");
