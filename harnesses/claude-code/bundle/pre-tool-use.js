...
import { join as join24, dirname as dirname12, sep } from "node:path";
...
import { readFileSync as readFileSync3, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, openSync, closeSync, unlinkSync as unlinkSync2, statSync } from "node:fs";
...
import { readFileSync as readFileSync4, writeFileSync as writeFileSync3, mkdirSync as mkdirSync4, unlinkSync as unlinkSync3, renameSync as renameSync3 } from "node:fs";
...
function isDirectRun(metaUrl) {
  ...
  if (!entry)
    return false;
  try {
    return resolve4(fileURLToPath2(metaUrl)) === resolve4(entry);
  } catch {
    return false;
  }
}
...
import { mkdirSync as mkdirSync11, readFileSync as readFileSync13, writeFileSync as writeFileSync9, renameSync as renameSync8, existsSync as existsSync10 } from "node:fs";
import { homedir as homedir10 } from "node:os";
import { createHash as createHash5 } from "node:crypto";
import { dirname as dirname10, join as join19 } from "node:path";
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join19(homedir10(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash5("sha256").update(`${project}\0${scope}`).digest("hex");
  return join19(privateStoreRoot(), `${key}.json`);
}
...
function resolveEmbedDaemonPath() {
  return join20(dirname11(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
import { mkdirSync as mkdirSync12, readFileSync as readFileSync14, rmSync, writeFileSync as writeFileSync10 } from "node:fs";
import { join as join21 } from "node:path";
import { homedir as homedir11 } from "node:os";
var log5 = (msg) => log("query-cache", msg);
var DEFAULT_CACHE_ROOT = join21(homedir11(), ".deeplake", "query-cache");
var INDEX_CACHE_FILE = "index.md";
function getSessionQueryCacheDir(sessionId, deps = {}) {
  const { cacheRoot = DEFAULT_CACHE_ROOT } = deps;
  return join21(cacheRoot, sessionId);
}
function readCachedIndexContent(sessionId, deps = {}) {
  const { logFn = log5 } = deps;
  try {
    return readFileSync14(join21(getSessionQueryCacheDir(sessionId, deps), INDEX_CACHE_FILE), "utf-8");
  } catch (e) {
    if (e?.code === "ENOENT")
      return null;
...
function writeCachedIndexContent(sessionId, content, deps = {}) {
  ...
  try {
    ...
    writeFileSync10(join21(dir, INDEX_CACHE_FILE), content, "utf-8");
  } catch (e) {
...
var MEMORY_PATH = join22(homedir12(), ".deeplake", "memory");
...
import { readFileSync as readFileSync15, writeFileSync as writeFileSync11, writeSync as writeSync2, mkdirSync as mkdirSync13, existsSync as existsSync11, unlinkSync as unlinkSync5, openSync as openSync3, closeSync as closeSync3, statSync as statSync3 } from "node:fs";
...
var __bundleDir = dirname12(fileURLToPath5(import.meta.url));
var READ_CACHE_ROOT = join24(homedir14(), ".deeplake", "query-cache");
function writeReadCacheFile(sessionId, virtualPath, content, deps = {}) {
  ...
  const expectedRoot = join24(cacheRoot, safeSessionId, "read");
  const absPath = join24(expectedRoot, rel);
  ...
  mkdirSync14(dirname12(absPath), { recursive: true });
