...
import { existsSync as existsSync8, mkdirSync as mkdirSync11, readFileSync as readFileSync14, writeFileSync as writeFileSync9 } from "node:fs";
...
// dist/src/commands/graph.js
import { readFileSync as readFileSync17, readdirSync } from "node:fs";
import { join as join20, relative as relative2, resolve as resolve5, sep } from "node:path";
...
var LOG = join13(homedir5(), ".deeplake", "hook-debug.log");
function isDebug() {
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync8(dirname8(LOG), { recursive: true });
...
import { readFileSync as readFileSync12, writeFileSync as writeFileSync7, mkdirSync as mkdirSync9, openSync, closeSync, unlinkSync as unlinkSync3, statSync as statSync2 } from "node:fs";
...
function queuePath() {
  return join14(homedir6(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
function writeQueue(q) {
  ...
  mkdirSync9(join14(home, ".deeplake"), { recursive: true, mode: 448 });
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync9(join14(homedir6(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { readFileSync as readFileSync13, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, unlinkSync as unlinkSync4, renameSync as renameSync5 } from "node:fs";
import { join as join15 } from "node:path";
import { homedir as homedir7 } from "node:os";
function configDir() {
  return join15(homedir7(), ".deeplake");
}
function credsPath() {
...
import { mkdirSync as mkdirSync13, readFileSync as readFileSync16, writeFileSync as writeFileSync11 } from "node:fs";
...
function loadGraphIgnore(deeplakeDir = join19(homedir8(), ".deeplake")) {
  const path = join19(deeplakeDir, FILE_NAME);
  try {
...
async function runBuildCommand(args) {
  ...
  for (const abs of sourceFiles) {
    ...
    try {
      const content = readFileSync17(abs, "utf8");
...
import { mkdirSync as mkdirSync14, readFileSync as readFileSync18, statSync as statSync3, unlinkSync as unlinkSync5, writeFileSync as writeFileSync12 } from "node:fs";
import { join as join21 } from "node:path";
var STALE_LOCK_MS = 5 * 60 * 1e3;
function lockPath2(baseDir) {
  return join21(baseDir, ".build.in-flight");
}
...
function isDirectRun(metaUrl) {
  ...
  if (!entry)
    return false;
  try {
    return resolve6(fileURLToPath2(metaUrl)) === resolve6(entry);
  } catch {
    return false;
  }
}
...
function logToFile(cwd, line) {
  try {
    ...
    appendFileSync3(join22(dir, ".graph-on-stop.log"), `[${(/* @__PURE__ */ new Date()).toISOString()}] ${line}
`);
  } catch {
  }
}
