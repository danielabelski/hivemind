...
import { existsSync as existsSync9, mkdirSync as mkdirSync12, readFileSync as readFileSync16, writeFileSync as writeFileSync10 } from "node:fs";
...
function hasFreshIndexMarker(markerPath) {
  if (!existsSync9(markerPath))
    return false;
  try {
    const raw = JSON.parse(readFileSync16(markerPath, "utf-8"));
...
function writeIndexMarker(markerPath) {
  mkdirSync12(getIndexMarkerDir(), { recursive: true });
...
import { existsSync as existsSync4, mkdirSync as mkdirSync4, readFileSync as readFileSync7, renameSync as renameSync3, writeFileSync as writeFileSync4 } from "node:fs";
import { dirname as dirname4, join as join8 } from "node:path";
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join8(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join8(baseDir, ".last-build.json");
}
...
import { mkdirSync as mkdirSync6, renameSync as renameSync4, writeFileSync as writeFileSync5 } from "node:fs";
import { homedir as homedir5 } from "node:os";
import { dirname as dirname6, join as join10 } from "node:path";

// dist/src/graph/history.js
import { appendFileSync, existsSync as existsSync5, mkdirSync as mkdirSync5, readFileSync as readFileSync8 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join10(homedir5(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join10(graphsRoot(), repoKey);
}
...
function writeSnapshot(snapshot, baseDir, trigger = "unknown", worktreeId) {
  ...
  const snapshotsDir = join10(baseDir, "snapshots");
  const snapshotPath = join10(snapshotsDir, `${fileBase}.json`);
  const canonical = canonicalSnapshot(snapshot);
  writeFileAtomic(snapshotPath, canonical);
  const worktreeRoot = worktreeId !== void 0 ? join10(baseDir, "worktrees", worktreeId) : baseDir;
  let latestCommitPath = null;
  if (commitSha !== null) {
    latestCommitPath = join10(worktreeRoot, "latest-commit.txt");
...
// dist/src/commands/graph.js
import { readFileSync as readFileSync19, readdirSync } from "node:fs";
import { join as join23, relative as relative2, resolve as resolve6, sep } from "node:path";
...
import { existsSync as existsSync7, mkdirSync as mkdirSync7, readFileSync as readFileSync10, writeFileSync as writeFileSync6, cpSync, symlinkSync, unlinkSync as unlinkSync2, lstatSync } from "node:fs";
...
function getVersion() {
  try {
    const pkg = JSON.parse(readFileSync11(join13(pkgRoot(), "package.json"), "utf-8"));
...
import { existsSync as existsSync8, mkdirSync as mkdirSync8, readFileSync as readFileSync12, renameSync as renameSync5, writeFileSync as writeFileSync7 } from "node:fs";
...
function cacheDir(baseDir) {
  return join14(baseDir, ".cache");
}
function cachePath(baseDir, contentSha256) {
  return join14(cacheDir(baseDir), `${contentSha256}.json`);
}
...
import { dirname as dirname9, join as join15, resolve as resolve3 } from "node:path";
...
function findDirConfig(startDir, stopAt) {
  ...
  for (; ; ) {
    for (const name of DIR_CONFIG_FILENAMES) {
      ...
      try {
        const raw = parseDirConfig(readFileSync13(candidate, "utf-8"));
        if (raw)
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync9(dirname10(LOG), { recursive: true });
...
import { readFileSync as readFileSync14, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, openSync, closeSync, unlinkSync as unlinkSync3, statSync as statSync2 } from "node:fs";
...
function writeQueue(q) {
  ...
  mkdirSync10(join17(home, ".deeplake"), { recursive: true, mode: 448 });
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync10(join17(homedir8(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { readFileSync as readFileSync15, writeFileSync as writeFileSync9, mkdirSync as mkdirSync11, unlinkSync as unlinkSync4, renameSync as renameSync6 } from "node:fs";
import { join as join18 } from "node:path";
import { homedir as homedir9 } from "node:os";
function configDir() {
  return join18(homedir9(), ".deeplake");
}
function credsPath() {
  return join18(configDir(), "credentials.json");
}
function loadCredentials(readFile = (p) => readFileSync15(p, "utf-8")) {
  try {
...
import { existsSync as existsSync10, mkdirSync as mkdirSync13, renameSync as renameSync7, writeFileSync as writeFileSync11 } from "node:fs";
import { dirname as dirname11, join as join20 } from "node:path";

// dist/src/graph/diff.js
import { existsSync as existsSync11, readFileSync as readFileSync17 } from "node:fs";
...
import { mkdirSync as mkdirSync14, readFileSync as readFileSync18, writeFileSync as writeFileSync12 } from "node:fs";
...
function loadGraphIgnore(deeplakeDir = join22(homedir10(), ".deeplake")) {
  const path = join22(deeplakeDir, FILE_NAME);
  try {
  ...
  try {
    mkdirSync14(deeplakeDir, { recursive: true });
    writeFileSync12(path, JSON.stringify(defaultConfigObject(), null, 2) + "\n", { flag: "wx" });
  } catch {
  }
...
import { mkdirSync as mkdirSync15, readFileSync as readFileSync20, statSync as statSync3, unlinkSync as unlinkSync5, writeFileSync as writeFileSync13 } from "node:fs";
import { join as join24 } from "node:path";
var STALE_LOCK_MS = 5 * 60 * 1e3;
function lockPath2(baseDir) {
  return join24(baseDir, ".build.in-flight");
}
function acquireBuildLock(baseDir) {
  const path = lockPath2(baseDir);
  try {
    mkdirSync15(baseDir, { recursive: true });
  } catch {
    return { acquired: false, reason: "fs-error" };
  }
  try {
    writeFileSync13(path, JSON.stringify({ pid: process.pid, ts: Date.now() }), { flag: "wx" });
...
function countSourceDiff(cwd, from, to) {
  if (from === null)
    return 1;
  try {
    const out = execFileSync6("git", ["diff", "--name-only", `${from}..${to}`, "--", ...SOURCE_GLOBS], {
      cwd,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    }).trim();
...
function logToFile(cwd, line) {
  try {
    ...
    appendFileSync3(join25(dir, ".graph-on-stop.log"), `[${(/* @__PURE__ */ new Date()).toISOString()}] ${line}
`);
  } catch {
  }
}
