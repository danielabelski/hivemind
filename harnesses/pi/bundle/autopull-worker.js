...
import { existsSync as existsSync7, readFileSync as readFileSync8, writeFileSync as writeFileSync6, mkdirSync as mkdirSync7, renameSync as renameSync4, rmSync, lstatSync as lstatSync2, readlinkSync, symlinkSync, unlinkSync as unlinkSync4 } from "node:fs";
import { homedir as homedir8 } from "node:os";
import { dirname as dirname5, join as join12 } from "node:path";

// dist/src/skillify/skill-writer.js
import { existsSync as existsSync3, mkdirSync as mkdirSync5, readFileSync as readFileSync6, readdirSync, statSync as statSync2, writeFileSync as writeFileSync4 } from "node:fs";
...
import { existsSync as existsSync5, lstatSync, mkdirSync as mkdirSync6, readFileSync as readFileSync7, renameSync as renameSync3, unlinkSync as unlinkSync3, writeFileSync as writeFileSync5 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join8(homedir6(), ".deeplake", "state", "skillify");
}
...
function manifestPath() {
  return join10(getStateDir(), "pulled.json");
}
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync5(join10(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync6(join11(home, ".codex"));
  const piInstalled = existsSync6(join11(home, ".pi", "agent"));
  const hermesInstalled = existsSync6(join11(home, ".hermes"));
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    const staleFile = staleOwned ? join12(root, staleDir, "SKILL.md") : null;
    const staleVersion = staleFile && existsSync7(staleFile) ? readLocalVersion(staleFile) : null;
    ...
    if (action === "wrote") {
      ...
      if (staleFile && staleVersion !== null && staleVersion >= remoteVersion && !(opts.force ?? false)) {
        try {
          const migrated = readFileSync8(staleFile, "utf-8").replace(/^name:.*$/m, `name: ${name}`);
          writeFileSync6(skillFile, migrated);
        } catch {
          writeFileSync6(skillFile, renderSkillFile(row, name));
        }
      } else {
        writeFileSync6(skillFile, renderSkillFile(row, name));
      }
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync7(skillFile) && existsSync7(join12(root, staleDir))) {
      try {
        ...
        if (canonicalRecorded && staleEntry) {
          rmSync(join12(root, staleDir), { recursive: true, force: true });
...
import { cpSync, existsSync as existsSync8, readFileSync as readFileSync9, rmSync as rmSync2, writeFileSync as writeFileSync7 } from "node:fs";
...
function readInstalledName(skillFile) {
  ...
  let text;
  try {
    text = readFileSync9(skillFile, "utf-8");
  } catch {
    return null;
  }
  const parsed = parseFrontmatter(text);
...
function retireEntry(entry) {
  ...
  rmSync2(join13(entry.installRoot, entry.dirName), { recursive: true, force: true });
  removePullEntry(entry.install, entry.installRoot, entry.dirName);
}
function migrateEntry(entry) {
  if (entry.install !== "global")
    return false;
  const staleDir = join13(entry.installRoot, entry.dirName);
  const staleFile = join13(staleDir, "SKILL.md");
  ...
  const cappedDirPath = join13(entry.installRoot, cappedDir);
  const cappedFile = join13(cappedDirPath, "SKILL.md");
  ...
  try {
    ...
    const migrated = readFileSync9(cappedFile, "utf-8").replace(/^name:.*$/m, `name: ${capped}`);
    writeFileSync7(cappedFile, migrated);
