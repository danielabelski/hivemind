...
import { existsSync as existsSync10, readFileSync as readFileSync11, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, renameSync as renameSync4, lstatSync as lstatSync2, readlinkSync, symlinkSync, unlinkSync as unlinkSync5 } from "node:fs";
...
import { existsSync as existsSync11, mkdirSync as mkdirSync11, readFileSync as readFileSync12, renameSync as renameSync5, writeFileSync as writeFileSync9 } from "node:fs";
...
import { appendFileSync as appendFileSync2, existsSync as existsSync12, mkdirSync as mkdirSync12, readFileSync as readFileSync13 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join21(homedir12(), ".hivemind", "graphs");
}
    ...
    try {
      ...
      if (config) {
        ...
        if (collectHere) {
          ...
          log6(dirRes && !dirRes.collect ? `placeholder + schema ensure skipped (.hivemind collect:false ${dirRes.found?.path})` : "placeholder + schema ensure skipped (HIVEMIND_CAPTURE=false)");
        }
        const known = await api.knownTablesOrNull();
