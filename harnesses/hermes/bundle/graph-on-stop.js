...
import { existsSync as existsSync8, mkdirSync as mkdirSync11, readFileSync as readFileSync15, writeFileSync as writeFileSync9 } from "node:fs";
...
function writeIndexMarker(markerPath) {
  mkdirSync11(getIndexMarkerDir(), { recursive: true });
...
import { chmodSync, existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync5, unlinkSync, writeFileSync as writeFileSync2 } from "node:fs";
...
import { existsSync as existsSync3, mkdirSync as mkdirSync3, readFileSync as readFileSync6, renameSync as renameSync2, writeFileSync as writeFileSync3 } from "node:fs";
...
import { mkdirSync as mkdirSync5, renameSync as renameSync3, writeFileSync as writeFileSync4 } from "node:fs";
import { homedir as homedir3 } from "node:os";
import { dirname as dirname5, join as join8 } from "node:path";

// dist/src/graph/history.js
import { appendFileSync, existsSync as existsSync4, mkdirSync as mkdirSync4, readFileSync as readFileSync7 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join8(homedir3(), ".hivemind", "graphs");
}
...
function writeSnapshot(snapshot, baseDir, trigger = "unknown", worktreeId) {
  ...
  const snapshotsDir = join8(baseDir, "snapshots");
  const snapshotPath = join8(snapshotsDir, `${fileBase}.json`);
  const canonical = canonicalSnapshot(snapshot);
  writeFileAtomic(snapshotPath, canonical);
  const worktreeRoot = worktreeId !== void 0 ? join8(baseDir, "worktrees", worktreeId) : baseDir;
  let latestCommitPath = null;
  if (commitSha !== null) {
    latestCommitPath = join8(worktreeRoot, "latest-commit.txt");
...
// dist/src/commands/graph.js
import { readFileSync as readFileSync18, readdirSync } from "node:fs";
import { join as join21, relative as relative2, resolve as resolve6, sep } from "node:path";
...
import { existsSync as existsSync6, mkdirSync as mkdirSync6, readFileSync as readFileSync9, writeFileSync as writeFileSync5, cpSync, symlinkSync, unlinkSync as unlinkSync2, lstatSync } from "node:fs";
...
function claudeDesktopConfigDir() {
  if (process.platform === "darwin")
    return join10(HOME, "Library", "Application Support", "Claude");
  if (process.platform === "win32")
    return join10(process.env.APPDATA ?? join10(HOME, "AppData", "Roaming"), "Claude");
  return join10(HOME, ".config", "Claude");
}
var PLATFORM_MARKERS = [
  { id: "claude", markerDir: join10(HOME, ".claude") },
  { id: "codex", markerDir: join10(HOME, ".codex") },
  { id: "claw", markerDir: join10(HOME, ".openclaw") },
  { id: "cursor", markerDir: join10(HOME, ".cursor") },
  { id: "hermes", markerDir: join10(HOME, ".hermes") },
  ...
  { id: "pi", markerDir: join10(HOME, ".pi") },
...
function getVersion() {
  try {
    const pkg = JSON.parse(readFileSync10(join11(pkgRoot(), "package.json"), "utf-8"));
...
import { existsSync as existsSync7, mkdirSync as mkdirSync7, readFileSync as readFileSync11, renameSync as renameSync4, writeFileSync as writeFileSync6 } from "node:fs";
import { dirname as dirname7, join as join12 } from "node:path";
var CACHE_SCHEMA_VERSION = 1;
function fileContentHash(contents) {
  return createHash5("sha256").update(contents).digest("hex");
}
function cacheDir(baseDir) {
  return join12(baseDir, ".cache");
}
function cachePath(baseDir, contentSha256) {
  return join12(cacheDir(baseDir), `${contentSha256}.json`);
}
...
function writeCache(baseDir, contentSha256, extraction) {
  ...
  try {
    ...
    const tmp = `${path}.tmp.${process.pid}.${Date.now()}`;
    writeFileSync6(tmp, JSON.stringify(entry));
...
import { dirname as dirname8, join as join13, resolve as resolve3 } from "node:path";
...
function findDirConfig(startDir, stopAt) {
  ...
  for (; ; ) {
    for (const name of DIR_CONFIG_FILENAMES) {
      ...
      try {
        const raw = parseDirConfig(readFileSync12(candidate, "utf-8"));
        if (raw)
...
function log(tag, msg) {
  if (!isDebug())
    return;
  try {
    mkdirSync8(dirname9(LOG), { recursive: true });
...
import { readFileSync as readFileSync13, writeFileSync as writeFileSync7, mkdirSync as mkdirSync9, openSync, closeSync, unlinkSync as unlinkSync3, statSync as statSync2 } from "node:fs";
...
import { resolve as resolve4, relative, isAbsolute } from "node:path";
function isPathInsideHome(path, home) {
  const r = resolve4(path);
  const h = resolve4(home);
  if (r === h)
    return true;
...
function queuePath() {
  return join15(homedir6(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync9(join15(homedir6(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { readFileSync as readFileSync14, writeFileSync as writeFileSync8, mkdirSync as mkdirSync10, unlinkSync as unlinkSync4, renameSync as renameSync5 } from "node:fs";
import { join as join16 } from "node:path";
import { homedir as homedir7 } from "node:os";
function configDir() {
  return join16(homedir7(), ".deeplake");
}
function credsPath() {
  return join16(configDir(), "credentials.json");
}
function loadCredentials(readFile = (p) => readFileSync14(p, "utf-8")) {
  try {
...
async function pushSnapshot(snapshot, worktreeId, deps = {}) {
  ...
  const canonical = canonicalJSON2(snapshot);
...
function computeSnapshotSha2562(snapshot) {
  ...
  return createHash6("sha256").update(canonicalJSON2(stable)).digest("hex");
}
...
import { existsSync as existsSync9, mkdirSync as mkdirSync12, renameSync as renameSync6, writeFileSync as writeFileSync10 } from "node:fs";
import { dirname as dirname10, join as join18 } from "node:path";

// dist/src/graph/diff.js
import { existsSync as existsSync10, readFileSync as readFileSync16 } from "node:fs";
...
import { mkdirSync as mkdirSync13, readFileSync as readFileSync17, writeFileSync as writeFileSync11 } from "node:fs";
...
function loadGraphIgnore(deeplakeDir = join20(homedir8(), ".deeplake")) {
  const path = join20(deeplakeDir, FILE_NAME);
  try {
  ...
  try {
    ...
    writeFileSync11(path, JSON.stringify(defaultConfigObject(), null, 2) + "\n", { flag: "wx" });
  } catch {
  }
...
async function runBuildCommand(args) {
  ...
  for (const abs of sourceFiles) {
    ...
    try {
      const content = readFileSync18(abs, "utf8");
...
function workTreeIdFor(cwd) {
  return createHash8("sha256").update(cwd).digest("hex").slice(0, 16);
}
...
import { mkdirSync as mkdirSync14, readFileSync as readFileSync19, statSync as statSync3, unlinkSync as unlinkSync5, writeFileSync as writeFileSync12 } from "node:fs";
import { join as join22 } from "node:path";
var STALE_LOCK_MS = 5 * 60 * 1e3;
function lockPath2(baseDir) {
  return join22(baseDir, ".build.in-flight");
}
function acquireBuildLock(baseDir) {
  const path = lockPath2(baseDir);
  try {
    mkdirSync14(baseDir, { recursive: true });
  } catch {
    return { acquired: false, reason: "fs-error" };
  }
  try {
    writeFileSync12(path, JSON.stringify({ pid: process.pid, ts: Date.now() }), { flag: "wx" });
...
function isDirectRun(metaUrl) {
  ...
  if (!entry)
    return false;
  try {
    return resolve7(fileURLToPath2(metaUrl)) === resolve7(entry);
  } catch {
    return false;
  }
}
...
function workTreeIdFor2(cwd) {
  return createHash9("sha256").update(cwd).digest("hex").slice(0, 16);
}
...
function countSourceDiff(cwd, from, to) {
  if (from === null)
    return 1;
  try {
    const out = execFileSync4("git", ["diff", "--name-only", `${from}..${to}`, "--", ...SOURCE_GLOBS], {
      cwd,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    }).trim();
...
function logToFile(cwd, line) {
  try {
    ...
    appendFileSync3(join23(dir, ".graph-on-stop.log"), `[${(/* @__PURE__ */ new Date()).toISOString()}] ${line}
`);
  } catch {
  }
}
