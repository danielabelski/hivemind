  ...
  switch (agent) {
    ...
    case "claude_code":
      return firstExistingPath([
        join16(home, ".claude", "local", "claude"),
        "/usr/local/bin/claude",
        "/usr/bin/claude",
        join16(home, ".npm-global", "bin", "claude"),
        join16(home, ".local", "bin", "claude"),
    ...
    case "codex":
      return firstExistingPath([
        "/usr/local/bin/codex",
        "/usr/bin/codex",
        join16(home, ".npm-global", "bin", "codex"),
        join16(home, ".local", "bin", "codex"),
    ...
    case "cursor":
      return firstExistingPath([
        ...
        join16(home, ".npm-global", "bin", "cursor-agent"),
        join16(home, ".local", "bin", "cursor-agent"),
    ...
    case "hermes":
      return firstExistingPath([
        ...
        "/usr/bin/hermes",
        join16(home, ".npm-global", "bin", "hermes"),
        "/opt/homebrew/bin/hermes"
      ]) ?? join16(home, ".local", "bin", "hermes");
    case "pi":
      return firstExistingPath([
        join16(home, ".local", "bin", "pi"),
        "/usr/local/bin/pi",
        "/usr/bin/pi",
        join16(home, ".npm-global", "bin", "pi"),
        "/opt/homebrew/bin/pi"
      ]) ?? join16(home, ".local", "bin", "pi");
  }
}
...
// dist/src/skillify/spawn-skillify-worker.js
var HOME2 = homedir13();
var SKILLIFY_LOG = join17(HOME2, ".claude", "hooks", "skillify.log");
function skillifyLog(msg) {
  try {
    mkdirSync10(dirname7(SKILLIFY_LOG), { recursive: true });
...
function spawnSkillifyWorker(opts) {
  ...
  const tmpDir = join17(tmpdir3(), `deeplake-skillify-${projectKey}-${Date.now()}`);
  ...
  const workerPath = join17(bundleDir, "skillify-worker.js");
  spawnDetachedNodeWorker(workerPath, [configFile]);
  skillifyLog(`${reason}: spawned skillify worker for ${projectKey}`);
}

// dist/src/skillify/state.js
import { readFileSync as readFileSync10, writeFileSync as writeFileSync8, writeSync as writeSync3, mkdirSync as mkdirSync11, renameSync as renameSync5, rmdirSync, existsSync as existsSync9, lstatSync as lstatSync2, unlinkSync as unlinkSync5, openSync as openSync4, closeSync as closeSync4 } from "node:fs";
...
function migrateLegacyStateDir() {
  ...
  const legacy = join19(dirname8(current), "skilify");
...
function statePath2(projectKey) {
  return join20(getStateDir(), `${projectKey}.json`);
}
function lockPath3(projectKey) {
  return join20(getStateDir(), `${projectKey}.lock`);
}
...
import { existsSync as existsSync10, mkdirSync as mkdirSync12, readFileSync as readFileSync11, writeFileSync as writeFileSync9 } from "node:fs";
...
function isHivemindPluginEnabled() {
  try {
    const settingsPath = join22(homedir15(), ".claude", "settings.json");
...
import { existsSync as existsSync11, lstatSync as lstatSync3, mkdirSync as mkdirSync13, readFileSync as readFileSync13, renameSync as renameSync6, unlinkSync as unlinkSync6, writeFileSync as writeFileSync10 } from "node:fs";
