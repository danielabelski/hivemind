...
function firstProseSentence(summary) {
...
async function pickResumeBrief(creds) {
  ...
  try {
    ...
    const rows = await withTimeout(api.query(`SELECT summary, project, last_update_date FROM "${table}" WHERE project = '${sqlStr(project)}' AND author = '${sqlStr(creds.userName)}' AND summary <> '' ORDER BY last_update_date DESC LIMIT 1`), QUERY_TIMEOUT_MS, null);
    if (!rows || rows.length === 0) {
      log9(`silent (no prior summary for project=${project})`);
      return null;
    }
    const summary = typeof rows[0].summary === "string" ? rows[0].summary : "";
    const line = truncate2(firstProseSentence(summary));
    if (line.length < 8)
      return null;
    const age = relativeAge(rows[0].last_update_date);
    const when = age ? ` (${age})` : "";
    const brief = `Picking up on ${project}${when} \u2014 last time you left off here:
   \u{1F4CC} ${line}
   Ask me for the full thread whenever you're ready.`;
    log9(`fired (project=${project})`);
    return { brief };
  } catch (e) {
