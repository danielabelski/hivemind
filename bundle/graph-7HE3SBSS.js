...
import { join as join5, relative, resolve as resolve2, sep } from "node:path";
...
async function pullSnapshot(cwd, deps = {}) {
  ...
  const snapshotPath = join3(snapshotsDir, `${head}.json`);
  const worktreeRoot = join3(baseDir, "worktrees", worktreeId);
  try {
    writeFileAtomic(snapshotPath, cloudPayload);
    writeFileAtomic(join3(worktreeRoot, "latest-commit.txt"), `${head}
`);
...
function loadGraphIgnore(deeplakeDir = join4(homedir(), ".deeplake")) {
  const path = join4(deeplakeDir, FILE_NAME);
  try {
