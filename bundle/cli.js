...
import { existsSync as existsSync31, mkdirSync as mkdirSync14, readFileSync as readFileSync23, renameSync as renameSync8, writeFileSync as writeFileSync18 } from "node:fs";
...
import { existsSync as existsSync33, mkdirSync as mkdirSync17, readFileSync as readFileSync26, writeFileSync as writeFileSync20, unlinkSync as unlinkSync11 } from "node:fs";
...
import { openSync as openSync2, closeSync as closeSync2, writeSync as writeSync2, unlinkSync as unlinkSync10, existsSync as existsSync32, readFileSync as readFileSync25 } from "node:fs";
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync32(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync32(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    if (hello.daemonPath !== this.daemonEntry && !existsSync32(hello.daemonPath)) {
      _recycledStuckDaemon = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync32(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync32(this.daemonEntry)) {
...
async function stageSession(input, opts) {
  ...
  if (!existsSync33(input.jsonlPath)) {
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync34, unlinkSync as unlinkSync12 } from "node:fs";
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync35(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
...
import { closeSync as closeSync4, existsSync as existsSync37, mkdirSync as mkdirSync19, openSync as openSync4, statSync as statSync8, unlinkSync as unlinkSync14 } from "node:fs";
...
import { closeSync as closeSync3, existsSync as existsSync36, mkdirSync as mkdirSync18, openSync as openSync3, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync13 } from "node:fs";
...
function findBundledCliPath() {
  try {
    ...
    return existsSync36(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
...
function hasLocalClaudeSessions() {
  if (!existsSync36(CLAUDE_PROJECTS_DIR))
    return false;
  let subdirs;
  try {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync8(join42(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  if (existsSync36(LOCAL_MANIFEST_PATH))
    return { triggered: false, reason: "manifest-exists" };
  if (existsSync36(LOCAL_MINE_LOCK_PATH)) {
    let stale = false;
    try {
  ...
  try {
    mkdirSync18(HIVEMIND_DIR4, { recursive: true });
  ...
  try {
    mkdirSync18(join42(HOME4, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync37(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync37(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
...
import { closeSync as closeSync5, existsSync as existsSync38, mkdirSync as mkdirSync20, openSync as openSync5, readFileSync as readFileSync28, realpathSync, unlinkSync as unlinkSync15, writeSync as writeSync3 } from "node:fs";
...
function detectInstallKind(argv1) {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync38(`${gitDir}${sep}.git`)) {
