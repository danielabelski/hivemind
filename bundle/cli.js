...
import { existsSync as existsSync21, readFileSync as readFileSync16, writeFileSync as writeFileSync14, mkdirSync as mkdirSync9, renameSync as renameSync5, lstatSync as lstatSync8, readlinkSync as readlinkSync2, symlinkSync, unlinkSync as unlinkSync8 } from "node:fs";
...
import { existsSync as existsSync23, readdirSync as readdirSync5, rmSync as rmSync7, statSync as statSync5 } from "node:fs";
...
import { existsSync as existsSync32, mkdirSync as mkdirSync14, readFileSync as readFileSync24, rmSync as rmSync8, writeFileSync as writeFileSync19 } from "node:fs";
...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-7HE3SBSS.js");
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-7HE3SBSS.js"));
    } catch (err) {
