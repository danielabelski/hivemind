...
import { copyFileSync as copyFileSync3, chmodSync, existsSync as existsSync10, lstatSync as lstatSync5, readdirSync, readFileSync as readFileSync7, readlinkSync, rmSync as rmSync6, statSync, unlinkSync as unlinkSync5 } from "node:fs";
...
function findHivemindInstalls(home = HOME) {
  ...
  for (const inst of fixed) {
    if (existsSync10(join10(inst.pluginDir, "bundle")))
      out.push(inst);
  }
  ...
  if (existsSync10(ccCache)) {
    ...
    for (const ver of entries) {
      ...
      const candidates = [join10(dir, "bundle"), join10(dir, "harnesses", "claude-code", "bundle")];
...
function isSharedDepsInstalled(sharedNodeModules = SHARED_NODE_MODULES) {
  return existsSync10(join10(sharedNodeModules, TRANSFORMERS_PKG));
}
...
function killEmbedDaemon(socketDir) {
  ...
  let pid = null;
  try {
    pid = Number.parseInt(readFileSync7(pidPath, "utf-8").trim(), 10);
  } catch {
  }
...
import { readFileSync as readFileSync8, writeFileSync as writeFileSync7, mkdirSync } from "node:fs";
...
function installIDPath() {
  return join11(configDir(), "install-id");
}
var UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function getOrCreateInstallID() {
  try {
    const value = readFileSync8(installIDPath(), "utf-8").trim();
...
import { dirname as dirname4, join as join20, resolve } from "node:path";

// dist/src/dashboard/data.js
import { existsSync as existsSync16, readFileSync as readFileSync12, readdirSync as readdirSync3, statSync as statSync2 } from "node:fs";
import { homedir as homedir7 } from "node:os";
import { join as join18 } from "node:path";

// dist/src/notifications/sources/org-stats.js
import { existsSync as existsSync12, mkdirSync as mkdirSync2, readFileSync as readFileSync9, writeFileSync as writeFileSync8 } from "node:fs";
...
function cacheFilePath() {
  return join13(homedir4(), ".deeplake", "hivemind-stats-cache.json");
}
...
function readCache(scopeKey) {
  if (!existsSync12(cacheFilePath()))
    return {};
  try {
    const parsed = JSON.parse(readFileSync9(cacheFilePath(), "utf-8"));
    if (!parsed || typeof parsed !== "object")
      return {};
...
function writeCache(scopeKey, data) {
  try {
    mkdirSync2(dirname(cacheFilePath()), { recursive: true });
    const body = { fetchedAt: Date.now(), scopeKey, data };
    writeFileSync8(cacheFilePath(), JSON.stringify(body), "utf-8");
  } catch (e) {
...
import { appendFileSync, existsSync as existsSync13, mkdirSync as mkdirSync3, readFileSync as readFileSync10, readdirSync as readdirSync2 } from "node:fs";
import { dirname as dirname2, join as join14 } from "node:path";
import { homedir as homedir5 } from "node:os";
var log4 = (msg) => log2("usage-tracker", msg);
function statsFilePath() {
  return join14(homedir5(), ".deeplake", "usage-stats.jsonl");
}
function readUsageRecords() {
  try {
    if (!existsSync13(statsFilePath()))
      return [];
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join14(homedir5(), ".claude", "skills");
  if (!existsSync13(dir))
    return 0;
...
import { readFileSync as readFileSync11, writeFileSync as writeFileSync9, writeSync, mkdirSync as mkdirSync4, renameSync as renameSync3, rmdirSync, existsSync as existsSync15, lstatSync as lstatSync6, unlinkSync as unlinkSync6, openSync, closeSync } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join15(homedir6(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join16(dirname3(current), "skilify");
...
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join18(homedir7(), ".hivemind", "graphs");
}
...
function resolveSnapshot(repoDir2) {
  const snapshotsDir = join18(repoDir2, "snapshots");
  if (!existsSync16(snapshotsDir))
    return null;
  let snapshotPath = null;
  const pointer = join18(repoDir2, "latest-commit.txt");
  if (existsSync16(pointer)) {
    try {
      const sha = readFileSync12(pointer, "utf-8").trim();
      if (sha) {
        const candidate = join18(snapshotsDir, `${sha}.json`);
  ...
  if (!snapshotPath) {
    try {
      const candidates = readdirSync3(snapshotsDir).filter((name) => name.endsWith(".json")).map((name) => {
        const full = join18(snapshotsDir, name);
  ...
  let raw;
  try {
    raw = readFileSync12(snapshotPath, "utf-8");
  } catch (e) {
...
async function loadDashboardData(opts = {}) {
  ...
  const repoDir2 = join18(opts.graphsHome ?? graphsRoot(), repoKey);
...
function findBinaryOnPath(name) {
  ...
  for (const dir of PATH.split(delimiter)) {
    ...
    for (const ext of exts) {
      const candidate = join19(dir, name + ext);
      try {
...
function defaultDashboardOutPath(repoKey) {
  return join20(homedir8(), ".hivemind", "dashboards", repoKey, "index.html");
}
...
`);
  ...
  try {
    mkdirSync5(dirname4(absOut), { recursive: true });
    writeFileSync10(absOut, html, "utf-8");
  } catch (e) {
...
import { readdirSync as readdirSync7, existsSync as existsSync29, readFileSync as readFileSync21, mkdirSync as mkdirSync12, renameSync as renameSync6 } from "node:fs";
import { homedir as homedir17 } from "node:os";
import { dirname as dirname9, join as join32 } from "node:path";

// dist/src/skillify/scope-config.js
import { existsSync as existsSync17, mkdirSync as mkdirSync6, readFileSync as readFileSync13, writeFileSync as writeFileSync11 } from "node:fs";
...
function loadScopeConfig() {
  ...
  if (!existsSync17(CONFIG_PATH3))
    return DEFAULT;
  try {
    const raw = JSON.parse(readFileSync13(CONFIG_PATH3, "utf-8"));
...
import { existsSync as existsSync21, readFileSync as readFileSync16, writeFileSync as writeFileSync14, mkdirSync as mkdirSync9, renameSync as renameSync5, rmSync as rmSync7, lstatSync as lstatSync8, readlinkSync as readlinkSync2, symlinkSync, unlinkSync as unlinkSync8 } from "node:fs";
import { homedir as homedir11 } from "node:os";
import { dirname as dirname6, join as join25 } from "node:path";

// dist/src/skillify/skill-writer.js
import { existsSync as existsSync18, mkdirSync as mkdirSync7, readFileSync as readFileSync14, readdirSync as readdirSync4, statSync as statSync4, writeFileSync as writeFileSync12 } from "node:fs";
...
function resolveSkillsRoot(install, cwd) {
  if (install === "global") {
    return join22(homedir9(), ".claude", "skills");
  }
  return join22(cwd, ".claude", "skills");
}

// dist/src/skillify/manifest.js
import { existsSync as existsSync19, lstatSync as lstatSync7, mkdirSync as mkdirSync8, readFileSync as readFileSync15, renameSync as renameSync4, unlinkSync as unlinkSync7, writeFileSync as writeFileSync13 } from "node:fs";
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync19(join23(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync20(join24(home, ".codex"));
  const piInstalled = existsSync20(join24(home, ".pi", "agent"));
  const hermesInstalled = existsSync20(join24(home, ".hermes"));
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join25(homedir11(), ".claude", "skills");
  if (!cwd)
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    const staleFile = staleOwned ? join25(root, staleDir, "SKILL.md") : null;
    const staleVersion = staleFile && existsSync21(staleFile) ? readLocalVersion(staleFile) : null;
    ...
    if (action === "wrote") {
      ...
      if (staleFile && staleVersion !== null && staleVersion >= remoteVersion && !(opts.force ?? false)) {
        try {
          const migrated = readFileSync16(staleFile, "utf-8").replace(/^name:.*$/m, `name: ${name}`);
          writeFileSync14(skillFile, migrated);
        } catch {
          writeFileSync14(skillFile, renderSkillFile(row, name));
        }
      } else {
        writeFileSync14(skillFile, renderSkillFile(row, name));
      }
    ...
    if (staleDir && staleDir !== dirName && !(opts.dryRun ?? false) && existsSync21(skillFile) && existsSync21(join25(root, staleDir))) {
      try {
        ...
        if (canonicalRecorded && staleEntry) {
          rmSync7(join25(root, staleDir), { recursive: true, force: true });
...
function readLocalSkill(skillsRoot, name) {
  ...
  const path = join26(skillsRoot, name, "SKILL.md");
  if (!existsSync22(path)) {
    throw new Error(`skill '${name}' not found at ${path}`);
  }
  const parsed = parseFrontmatter(readFileSync17(path, "utf-8"));
  if (!parsed) {
...
async function runPush(args) {
  ...
  const localPath = join26(skillsRoot, args.skillName, "SKILL.md");
...
import { existsSync as existsSync23, readdirSync as readdirSync5, rmSync as rmSync8, statSync as statSync5 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { join as join27 } from "node:path";
function resolveUnpullRoot(install, cwd) {
  if (install === "global")
    return join27(homedir12(), ".claude", "skills");
  if (!cwd)
...
function runUnpull(opts) {
  ...
  if (existsSync23(root) && (opts.all || opts.legacyCleanup)) {
...
import { existsSync as existsSync28, mkdirSync as mkdirSync11, readFileSync as readFileSync20, writeFileSync as writeFileSync16 } from "node:fs";
import { homedir as homedir16 } from "node:os";
import { basename, dirname as dirname8, join as join31 } from "node:path";

// dist/src/skillify/local-source.js
import { readdirSync as readdirSync6, readFileSync as readFileSync18, existsSync as existsSync24, statSync as statSync6 } from "node:fs";
...
function collectJsonlRecursive(out, agent, dir, inCwd) {
  ...
  for (const e of entries) {
    if (e.isDirectory())
      collectJsonlRecursive(out, agent, join28(dir, e.name), inCwd);
    else if (e.isFile())
  ...
  for (const install of installs) {
    ...
    for (const entry of top) {
      ...
      if (entry.isDirectory()) {
        collectJsonlRecursive(out, install.agent, join28(install.sessionRoot, entry.name), inCwd);
...
import { existsSync as existsSync26, mkdirSync as mkdirSync10, readFileSync as readFileSync19, writeFileSync as writeFileSync15 } from "node:fs";
import { homedir as homedir15 } from "node:os";
import { dirname as dirname7, join as join30 } from "node:path";
var LOCAL_MANIFEST_PATH = join30(homedir15(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join30(homedir15(), ".claude", "hivemind", "local-mined.lock");
...
async function runAdvisor(manifestPath2 = LOCAL_MANIFEST_PATH) {
  ...
  if (!claudeBin || !existsSync27(claudeBin)) {
    return null;
  }
...
async function runMineLocalImpl(args) {
  ...
  const tmpDir = join31(homedir16(), ".claude", "hivemind", `mine-local-${Date.now()}`);
  ...
  const results = await parallelMap(picked, GATE_CONCURRENCY, async (s) => {
    ...
    const verdictText = existsSync28(verdictPath) ? readFileSync20(verdictPath, "utf-8") : gate.stdout;
...
function showStatus() {
  ...
  for (const f of files) {
    try {
      const s = JSON.parse(readFileSync21(join32(dir, f), "utf-8"));
...
function setInstall(loc) {
  ...
  const path = loc === "global" ? join32(homedir17(), ".claude", "skills") : "<cwd>/.claude/skills";
...
function promoteSkill(name, cwd) {
  ...
  const globalPath = join32(homedir17(), ".claude", "skills", name);
  if (!existsSync29(join32(projectPath, "SKILL.md"))) {
    console.error(`Skill '${name}' not found at ${projectPath}/SKILL.md`);
    process.exit(1);
  }
  if (existsSync29(join32(globalPath, "SKILL.md"))) {
  ...
  mkdirSync12(dirname9(globalPath), { recursive: true });
...
async function pullSkills(args) {
  ...
  const dest = toRaw === "global" ? join32(homedir17(), ".claude", "skills") : join32(process.cwd(), ".claude", "skills");
...
async function pushSkills(args) {
  ...
  const src = fromRaw === "global" ? join32(homedir17(), ".claude", "skills") : join32(process.cwd(), ".claude", "skills");
...
import { existsSync as existsSync31, readFileSync as readFileSync23, writeFileSync as writeFileSync18 } from "node:fs";
...
import { mkdirSync as mkdirSync13, readFileSync as readFileSync22, writeFileSync as writeFileSync17, renameSync as renameSync7, existsSync as existsSync30 } from "node:fs";
import { homedir as homedir18 } from "node:os";
import { createHash } from "node:crypto";
import { dirname as dirname10, join as join33 } from "node:path";
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join33(homedir18(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash("sha256").update(`${project}\0${scope}`).digest("hex");
  return join33(privateStoreRoot(), `${key}.json`);
}
function readMap(file) {
  try {
    if (!existsSync30(file))
      return {};
    const raw = JSON.parse(readFileSync22(file, "utf-8"));
...
function writeMap(file, map2) {
  mkdirSync13(dirname10(file), { recursive: true });
  const tmp = `${file}.${process.pid}.tmp`;
  writeFileSync17(tmp, JSON.stringify(map2, null, 1) + "\n");
...
async function runLocalWikiRefresh(args) {
  ...
  for (const group of selectWikiGroups(args.snap)) {
    ...
    const abs = join34(args.repoRoot, localFile);
    if (!existsSync31(abs)) {
    ...
    let current;
    try {
      current = readFileSync23(abs, "utf-8");
    } catch {
    ...
    writeFileSync18(abs, next.endsWith("\n") ? next : next + "\n");
...
import { existsSync as existsSync32, mkdirSync as mkdirSync14, readFileSync as readFileSync24, rmSync as rmSync9, writeFileSync as writeFileSync19 } from "node:fs";
import { dirname as dirname11, join as join35, resolve as resolve2, sep } from "node:path";
...
function readPullManifest(repoRoot) {
  try {
    const raw = JSON.parse(readFileSync24(join35(repoRoot, PULL_MANIFEST_DIR, PULL_MANIFEST_FILE), "utf-8"));
...
function writePullManifest(repoRoot, manifest) {
  const dir = join35(repoRoot, PULL_MANIFEST_DIR);
  mkdirSync14(dir, { recursive: true });
  writeFileSync19(join35(dir, PULL_MANIFEST_FILE), JSON.stringify(manifest, null, 2) + "\n");
}
function ensureGitignoreEntries(repoRoot) {
  const path = join35(repoRoot, ".gitignore");
  let current = "";
  try {
    current = readFileSync24(path, "utf-8");
  } catch {
  }
  ...
  writeFileSync19(path, prefix + missing.join("\n") + "\n");
  return true;
}
async function pullDocs(args) {
  ...
  for (const doc of latest.values()) {
    ...
    let existing = null;
    try {
      existing = readFileSync24(abs, "utf-8");
    } catch {
    }
    if (existing === body) {
      unchanged++;
    ...
    mkdirSync14(dirname11(abs), { recursive: true });
    writeFileSync19(abs, body);
    written.push(rel);
  }
...
import { openSync as openSync2, closeSync as closeSync2, writeSync as writeSync2, unlinkSync as unlinkSync10, existsSync as existsSync33, readFileSync as readFileSync25 } from "node:fs";
import { homedir as homedir19 } from "node:os";
import { join as join36 } from "node:path";
var SHARED_DAEMON_PATH2 = join36(homedir19(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync33(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync33(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    if (hello.daemonPath !== this.daemonEntry && !existsSync33(hello.daemonPath)) {
      _recycledStuckDaemon = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync25(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync33(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync33(this.daemonEntry)) {
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync25(this.pidPath, "utf-8").trim();
...
function defaultResolveTransformers() {
  const sharedDir = join37(homedir20(), ".hivemind", "embed-deps");
  try {
...
function resolveEmbedDaemonPath() {
  return join38(dirname12(fileURLToPath(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
function gitHeadOf(cwd) {
  try {
    return execFileSync4("git", ["-C", cwd, "rev-parse", "HEAD"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim() || null;
  } catch {
    return null;
  }
}
function hookInstalledAt(cwd) {
  ...
  if (!p)
    return false;
  try {
    return existsSync34(p) && containsOurMarkers(readFileSync26(p, "utf8"));
  } catch {
    return false;
  }
}
...
function resolveContent(positionalContent, args) {
  ...
  if (file !== void 0)
    return readFileSync26(file, "utf-8");
...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "wiki-refresh") {
    ...
    const git = (gitArgs) => {
      try {
        return execFileSync4("git", ["-C", cwd, ...gitArgs], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
      } catch {
        return null;
      }
    };
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-ENH4XWRM.js");
...
import { existsSync as existsSync35, mkdirSync as mkdirSync15, readFileSync as readFileSync27, renameSync as renameSync8, writeFileSync as writeFileSync20 } from "node:fs";
import { homedir as homedir21 } from "node:os";
import { dirname as dirname13, join as join39 } from "node:path";
var HIVEMIND_DIR3 = join39(homedir21(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join39(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join39(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join39(HIVEMIND_DIR3, "pending-memory.lock");
function readPendingMemoryManifest(path = PENDING_MEMORY_MANIFEST_PATH) {
  if (!existsSync35(path))
    return null;
  try {
    return JSON.parse(readFileSync27(path, "utf-8"));
  } catch {
    return null;
  }
}
function writePendingMemoryManifest(m, path = PENDING_MEMORY_MANIFEST_PATH) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync20(tmp, JSON.stringify(m, null, 2));
...
import { existsSync as existsSync36, mkdirSync as mkdirSync18, readFileSync as readFileSync29, writeFileSync as writeFileSync22, unlinkSync as unlinkSync11 } from "node:fs";
import { join as join43 } from "node:path";

// dist/src/hooks/spawn-wiki-worker.js
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { dirname as dirname15, join as join42 } from "node:path";
import { writeFileSync as writeFileSync21, mkdirSync as mkdirSync17 } from "node:fs";
import { homedir as homedir22, tmpdir } from "node:os";

// dist/src/utils/wiki-log.js
import { mkdirSync as mkdirSync16, appendFileSync as appendFileSync2 } from "node:fs";
import { join as join40 } from "node:path";
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join40(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir22();
var wikiLogger = makeWikiLogger(join42(HOME3, ".claude", "hooks"));
...
function countLines(path) {
  try {
    const buf = readFileSync29(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join43(stagingDir, `${key}.md`);
  const embeddingPath = join43(stagingDir, `${key}.embedding.json`);
  if (!existsSync36(input.jsonlPath)) {
    return { sessionId: key, ok: false, embedded: false, reason: "jsonl-missing" };
  }
  try {
    mkdirSync18(stagingDir, { recursive: true });
  } catch {
  ...
  const text = readFileSync29(summaryPath, "utf-8");
  if (!text.trim()) {
  ...
  if (!opts.skipEmbed) {
    try {
      ...
      if (vec) {
        writeFileSync22(embeddingPath, JSON.stringify(vec));
        embedded = true;
      }
    } catch {
    }
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync37, unlinkSync as unlinkSync12 } from "node:fs";
...
import { existsSync as existsSync38, readFileSync as readFileSync30 } from "node:fs";
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync30(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync38(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync30(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
...
import { closeSync as closeSync4, existsSync as existsSync40, mkdirSync as mkdirSync20, openSync as openSync4, statSync as statSync8, unlinkSync as unlinkSync14 } from "node:fs";
import { spawn as spawn7 } from "node:child_process";
import { homedir as homedir25 } from "node:os";
import { join as join45 } from "node:path";

// dist/src/skillify/spawn-mine-local-worker.js
import { execFileSync as execFileSync5, spawn as spawn6 } from "node:child_process";
import { closeSync as closeSync3, existsSync as existsSync39, mkdirSync as mkdirSync19, openSync as openSync3, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync13 } from "node:fs";
import { homedir as homedir24 } from "node:os";
import { dirname as dirname16, join as join44 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var HOME4 = homedir24();
var HIVEMIND_DIR4 = join44(HOME4, ".claude", "hivemind");
var LOG_PATH = join44(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join44(HOME4, ".claude", "projects");
var LOCK_STALE_MS = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname16(fileURLToPath3(import.meta.url));
    const cliPath = join44(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync39(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
function findHivemindLauncher() {
  ...
  try {
    ...
    const out = execFileSync5(lookup, ["hivemind"], {
      encoding: "utf-8",
...
function hasLocalClaudeSessions() {
  if (!existsSync39(CLAUDE_PROJECTS_DIR))
    return false;
  let subdirs;
  try {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync8(join44(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  if (existsSync39(LOCAL_MANIFEST_PATH))
    return { triggered: false, reason: "manifest-exists" };
  if (existsSync39(LOCAL_MINE_LOCK_PATH)) {
    let stale = false;
    try {
      const stats = statSync7(LOCAL_MINE_LOCK_PATH);
  ...
  try {
    mkdirSync19(HIVEMIND_DIR4, { recursive: true });
  ...
  try {
    mkdirSync19(join44(HOME4, ".claude", "hooks"), { recursive: true });
...
var LOCK_STALE_MS2 = 30 * 60 * 1e3;
function runBackfillGuards(deps) {
  if (deps.manifestExists())
    return { triggered: false, reason: "manifest-exists" };
  if (deps.lockExists()) {
    if (deps.lockAgeMs() <= LOCK_STALE_MS2)
      return { triggered: false, reason: "lock-exists" };
    if (!deps.removeLock())
      return { triggered: false, reason: "lock-exists" };
  }
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir25();
var HIVEMIND_DIR5 = join45(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join45(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync20(join45(HOME5, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync40(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync40(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
        return Date.now() - statSync8(PENDING_MEMORY_LOCK_PATH).mtimeMs;
      } catch {
        return 0;
      }
    },
    ...
    acquireLock: () => {
      try {
        mkdirSync20(HIVEMIND_DIR5, { recursive: true });
...
import { closeSync as closeSync5, existsSync as existsSync41, mkdirSync as mkdirSync21, openSync as openSync5, readFileSync as readFileSync31, realpathSync, unlinkSync as unlinkSync15, writeSync as writeSync3 } from "node:fs";
import { homedir as homedir26 } from "node:os";
import { dirname as dirname17, join as join46, sep as sep2 } from "node:path";
...
function defaultLockPath() {
  return join46(homedir26(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync41(`${gitDir}${sep2}.git`)) {
...
function tryAcquireLock(path) {
  mkdirSync21(dirname17(path), { recursive: true, mode: 448 });
  const claim = () => {
  ...
  let holderPid = 0;
  try {
    holderPid = Number(readFileSync31(path, "utf-8").trim()) || 0;
  } catch {
    try {
      return claim();
    } catch {
...
async function runInstallAll(args) {
  ...
  await runInstallDocsOnboarding({
    ...
    onboard: async ({ root, orgId, orgName }) => {
      const { runDocsOnboarding: runDocsOnboarding2 } = await import("./onboarding-XTXVES7S.js");
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-ENH4XWRM.js"));
    } catch (err) {
