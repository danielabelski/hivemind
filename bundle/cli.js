...
function resolveSnapshot(repoDir) {
  const snapshotsDir = join18(repoDir, "snapshots");
  if (!existsSync16(snapshotsDir))
    return null;
  let snapshotPath = null;
  const pointer = join18(repoDir, "latest-commit.txt");
...
async function loadDashboardData(opts = {}) {
  ...
  const repoDir = join18(opts.graphsHome ?? graphsRoot(), repoKey);
...
function makeHandle(server, host, port) {
  let resolveStopped;
  const stopped = new Promise((resolve2) => {
    resolveStopped = resolve2;
  });
...
async function renderContextBlock(query, input, opts = {}) {
  ...
  try {
    ...
    if (tableExists && !tableExists(input.rulesTable)) {
      log6(`render-context-block: rules table "${input.rulesTable}" not present \u2014 skipping read`);
    } else {
      try {
        ...
        log6(`render-context-block: rules unavailable (continuing): ${rmsg}`);
      }
    }
    let goals = [];
    if (tableExists && !tableExists(input.goalsTable)) {
      log6(`render-context-block: goals table "${input.goalsTable}" not present \u2014 skipping read`);
    } else {
      try {
        ...
        log6(`render-context-block: goals unavailable (continuing): ${gmsg}`);
      }
    }
...
import { existsSync as existsSync30, mkdirSync as mkdirSync13, readFileSync as readFileSync22, renameSync as renameSync7, writeFileSync as writeFileSync17 } from "node:fs";
import { homedir as homedir18 } from "node:os";
import { dirname as dirname10, join as join33 } from "node:path";
var HIVEMIND_DIR3 = join33(homedir18(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join33(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join33(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join33(HIVEMIND_DIR3, "pending-memory.lock");
...
function writePendingMemoryManifest(m, path = PENDING_MEMORY_MANIFEST_PATH) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync17(tmp, JSON.stringify(m, null, 2));
...
import { existsSync as existsSync31, mkdirSync as mkdirSync16, readFileSync as readFileSync24, writeFileSync as writeFileSync19, unlinkSync as unlinkSync10 } from "node:fs";
import { join as join37 } from "node:path";

// dist/src/hooks/spawn-wiki-worker.js
import { fileURLToPath } from "node:url";
import { dirname as dirname12, join as join36 } from "node:path";
import { writeFileSync as writeFileSync18, mkdirSync as mkdirSync15 } from "node:fs";
import { homedir as homedir19, tmpdir } from "node:os";

// dist/src/utils/wiki-log.js
import { mkdirSync as mkdirSync14, appendFileSync as appendFileSync2 } from "node:fs";
import { join as join34 } from "node:path";
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join34(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir19();
var wikiLogger = makeWikiLogger(join36(HOME3, ".claude", "hooks"));
...
function countLines(path) {
  try {
    const buf = readFileSync24(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join37(stagingDir, `${key}.md`);
  const embeddingPath = join37(stagingDir, `${key}.embedding.json`);
  if (!existsSync31(input.jsonlPath)) {
    return { sessionId: key, ok: false, embedded: false, reason: "jsonl-missing" };
  }
  try {
    mkdirSync16(stagingDir, { recursive: true });
  } catch {
  ...
  const text = readFileSync24(summaryPath, "utf-8");
  if (!text.trim()) {
  ...
  if (!opts.skipEmbed) {
    try {
      ...
      if (vec) {
        writeFileSync19(embeddingPath, JSON.stringify(vec));
        embedded = true;
      }
    } catch {
    }
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync32, unlinkSync as unlinkSync11 } from "node:fs";
...
import { existsSync as existsSync33, readFileSync as readFileSync25 } from "node:fs";
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync25(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync33(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync25(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
...
import { closeSync as closeSync3, existsSync as existsSync35, mkdirSync as mkdirSync18, openSync as openSync3, statSync as statSync8, unlinkSync as unlinkSync13 } from "node:fs";
import { spawn as spawn6 } from "node:child_process";
import { homedir as homedir22 } from "node:os";
import { join as join39 } from "node:path";

// dist/src/skillify/spawn-mine-local-worker.js
import { execFileSync as execFileSync4, spawn as spawn5 } from "node:child_process";
import { closeSync as closeSync2, existsSync as existsSync34, mkdirSync as mkdirSync17, openSync as openSync2, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync12 } from "node:fs";
import { homedir as homedir21 } from "node:os";
import { dirname as dirname13, join as join38 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
var HOME4 = homedir21();
var HIVEMIND_DIR4 = join38(HOME4, ".claude", "hivemind");
var LOG_PATH = join38(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join38(HOME4, ".claude", "projects");
var LOCK_STALE_MS = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname13(fileURLToPath2(import.meta.url));
    const cliPath = join38(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync34(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
function findHivemindLauncher() {
  ...
  try {
    ...
    const out = execFileSync4(lookup, ["hivemind"], {
      encoding: "utf-8",
...
function hasLocalClaudeSessions() {
  if (!existsSync34(CLAUDE_PROJECTS_DIR))
    return false;
  let subdirs;
  try {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync8(join38(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  if (existsSync34(LOCAL_MANIFEST_PATH))
    return { triggered: false, reason: "manifest-exists" };
  if (existsSync34(LOCAL_MINE_LOCK_PATH)) {
    let stale = false;
    try {
  ...
  try {
    mkdirSync17(HIVEMIND_DIR4, { recursive: true });
  ...
  try {
    mkdirSync17(join38(HOME4, ".claude", "hooks"), { recursive: true });
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir22();
var HIVEMIND_DIR5 = join39(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join39(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync18(join39(HOME5, ".claude", "hooks"), { recursive: true });
  const out = openSync3(LOG_PATH2, "a");
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync35(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync35(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
    ...
    removeLock: () => {
      try {
        unlinkSync13(PENDING_MEMORY_LOCK_PATH);
        return true;
      } catch {
        return false;
      }
    ...
    acquireLock: () => {
      try {
        mkdirSync18(HIVEMIND_DIR5, { recursive: true });
        closeSync3(openSync3(PENDING_MEMORY_LOCK_PATH, "wx"));
        return true;
      } catch {
        return false;
      }
...
import { existsSync as existsSync36, mkdirSync as mkdirSync19, writeFileSync as writeFileSync20 } from "node:fs";
...
function docsHintSentinelPath() {
  return process.env.HIVEMIND_DOCS_HINT_FILE ?? join40(homedir23(), ".deeplake", ".docs-hint-shown");
}
function docsHintShown(file = docsHintSentinelPath()) {
  return existsSync36(file);
}
function markDocsHintShown(file = docsHintSentinelPath()) {
  try {
    mkdirSync19(dirname14(file), { recursive: true });
    writeFileSync20(file, (/* @__PURE__ */ new Date()).toISOString() + "\n");
  } catch {
  }
}
...
import { closeSync as closeSync4, existsSync as existsSync37, mkdirSync as mkdirSync20, openSync as openSync4, readFileSync as readFileSync26, realpathSync, unlinkSync as unlinkSync14, writeSync as writeSync2 } from "node:fs";
import { homedir as homedir24 } from "node:os";
import { dirname as dirname15, join as join41, sep } from "node:path";
...
function defaultLockPath() {
  return join41(homedir24(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  const realArgv1 = (() => {
    try {
      return realpathSync(argv1 ?? process.argv[1] ?? fileURLToPath3(import.meta.url));
    } catch {
      return argv1 ?? process.argv[1] ?? fileURLToPath3(import.meta.url);
    }
  })();
  let dir = dirname15(realArgv1);
  let installDir = null;
  for (let i = 0; i < 10; i++) {
  ...
  installDir ??= dirname15(realArgv1);
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync37(`${gitDir}${sep}.git`)) {
      return { kind: "local-dev", installDir };
    }
    const parent = dirname15(gitDir);
    if (parent === gitDir)
      break;
    gitDir = parent;
  }
...
function tryAcquireLock(path) {
  mkdirSync20(dirname15(path), { recursive: true, mode: 448 });
  const claim = () => {
  ...
  let holderPid = 0;
  try {
    holderPid = Number(readFileSync26(path, "utf-8").trim()) || 0;
  } catch {
    try {
      return claim();
    } catch {
...
async function runUpdate(opts = {}) {
  ...
  const spawn7 = opts.spawn ?? defaultSpawn;
  switch (detected.kind) {
    case "npm-global": {
      ...
      try {
        ...
        try {
          spawn7("npm", ["install", "-g", `${PKG_NAME}@${latest}`]);
        } catch (e) {
        ...
        try {
          spawn7("hivemind", ["install", "--skip-auth"]);
        } catch (e) {
...
async function runInstallAll(args) {
  ...
  let inGitRepo = false;
  try {
    const { tryGitTopLevel } = await import("./git-hook-install-KB5REAXY.js");
  ...
  if (promptDocs) {
    ...
    try {
      const { runDocsCommand: runDocsCommand2 } = await import("./docs-A5D4CGC4.js");
      await runDocsCommand2(["sync", "--cwd", docsCwd]);
    } catch (err) {
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-JCAYBZ4M.js"));
    } catch (err) {
