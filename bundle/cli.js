...
var AGENTS_MD2 = join10(PI_AGENT_DIR, "AGENTS.md");
...
function installPi() {
  ...
  const prior = existsSync9(AGENTS_MD2) ? readFileSync7(AGENTS_MD2, "utf-8") : null;
...
function uninstallPi() {
  ...
  if (existsSync9(AGENTS_MD2)) {
    const prior = readFileSync7(AGENTS_MD2, "utf-8");
...
import { readFileSync as readFileSync11, writeFileSync as writeFileSync9, mkdirSync as mkdirSync4, unlinkSync as unlinkSync6, renameSync as renameSync3 } from "node:fs";
...
function loadCredentials(readFile = (p) => readFileSync11(p, "utf-8")) {
  try {
...
import { readFileSync as readFileSync13, writeFileSync as writeFileSync10, mkdirSync as mkdirSync6, openSync, closeSync, unlinkSync as unlinkSync7, statSync as statSync2 } from "node:fs";
...
import { join as join28, relative as relative2, resolve as resolve5, sep } from "node:path";
...
import { dirname as dirname13, join as join36, resolve as resolve6 } from "node:path";
...
import { readdirSync as readdirSync8, existsSync as existsSync37, readFileSync as readFileSync34, mkdirSync as mkdirSync25, renameSync as renameSync12 } from "node:fs";
...
function readLocalSkill(skillsRoot, name) {
  assertValidSkillName(name);
  const path = join42(skillsRoot, name, "SKILL.md");
  if (!existsSync31(path)) {
    throw new Error(`skill '${name}' not found at ${path}`);
  }
  const parsed = parseFrontmatter(readFileSync30(path, "utf-8"));
  if (!parsed) {
...
async function runPush(args) {
  ...
  const localPath = join42(skillsRoot, args.skillName, "SKILL.md");
...
import { existsSync as existsSync32, readdirSync as readdirSync6, rmSync as rmSync7, statSync as statSync6 } from "node:fs";
import { homedir as homedir20 } from "node:os";
import { join as join43 } from "node:path";
function resolveUnpullRoot(install, cwd) {
  if (install === "global")
    return join43(homedir20(), ".claude", "skills");
  if (!cwd)
    throw new Error("cwd required when install === 'project'");
  return join43(cwd, ".claude", "skills");
}
function runUnpull(opts) {
  ...
  for (const entry of entries) {
    summary.scanned++;
    const path = join43(root, entry.dirName);
  ...
  if (existsSync32(root) && (opts.all || opts.legacyCleanup)) {
    ...
    for (const dirName of readdirSync6(root)) {
      if (manifestDirNames.has(dirName))
        continue;
      const path = join43(root, dirName);
      let st;
      try {
        st = statSync6(path);
      } catch {
...
import { existsSync as existsSync36, mkdirSync as mkdirSync24, readFileSync as readFileSync33, writeFileSync as writeFileSync26 } from "node:fs";
import { homedir as homedir24 } from "node:os";
import { basename as basename2, dirname as dirname17, join as join47 } from "node:path";

// dist/src/skillify/local-source.js
import { readdirSync as readdirSync7, readFileSync as readFileSync31, existsSync as existsSync33, statSync as statSync7 } from "node:fs";
...
function detectInstalledAgents() {
  const installs = [];
  const claudeRoot = join44(HOME2, ".claude", "projects");
  if (existsSync33(claudeRoot)) {
    installs.push({
      agent: "claude_code",
  ...
  const codexRoot = join44(HOME2, ".codex", "sessions");
  if (existsSync33(codexRoot)) {
    installs.push({
      agent: "codex",
...
function pushJsonlFile(out, agent, dir, fileName, inCwd) {
  if (!fileName.endsWith(".jsonl"))
    return;
  const fullPath = join44(dir, fileName);
  let stats;
  try {
...
function collectJsonlRecursive(out, agent, dir, inCwd) {
  ...
  for (const e of entries) {
    if (e.isDirectory())
      collectJsonlRecursive(out, agent, join44(dir, e.name), inCwd);
    else if (e.isFile())
...
function listLocalSessions(installs, cwd) {
  ...
  for (const install of installs) {
    ...
    for (const entry of top) {
      ...
      if (entry.isDirectory()) {
        collectJsonlRecursive(out, install.agent, join44(install.sessionRoot, entry.name), inCwd);
...
function nativeJsonlToRows(filePath, sessionId, agent) {
  let raw;
  try {
    raw = readFileSync31(filePath, "utf-8");
  } catch {
    return [];
  }
  const rows = [];
  ...
  switch (agent) {
    ...
    case "claude_code":
      return firstExistingPath([
        join45(home, ".claude", "local", "claude"),
        "/usr/local/bin/claude",
        "/usr/bin/claude",
        join45(home, ".npm-global", "bin", "claude"),
        join45(home, ".local", "bin", "claude"),
    ...
    case "codex":
      return firstExistingPath([
        "/usr/local/bin/codex",
        "/usr/bin/codex",
        join45(home, ".npm-global", "bin", "codex"),
        join45(home, ".local", "bin", "codex"),
    ...
    case "cursor":
      return firstExistingPath([
        ...
        join45(home, ".npm-global", "bin", "cursor-agent"),
        join45(home, ".local", "bin", "cursor-agent"),
        "/opt/homebrew/bin/cursor-agent"
      ]) ?? "/usr/local/bin/cursor-agent";
    case "hermes":
      return firstExistingPath([
        join45(home, ".local", "bin", "hermes"),
        "/usr/local/bin/hermes",
        "/usr/bin/hermes",
        join45(home, ".npm-global", "bin", "hermes"),
        "/opt/homebrew/bin/hermes"
      ]) ?? join45(home, ".local", "bin", "hermes");
    case "pi":
      return firstExistingPath([
        join45(home, ".local", "bin", "pi"),
        "/usr/local/bin/pi",
        "/usr/bin/pi",
        join45(home, ".npm-global", "bin", "pi"),
        "/opt/homebrew/bin/pi"
      ]) ?? join45(home, ".local", "bin", "pi");
  }
}
...
import { existsSync as existsSync35, mkdirSync as mkdirSync23, readFileSync as readFileSync32, writeFileSync as writeFileSync25 } from "node:fs";
import { homedir as homedir23 } from "node:os";
import { dirname as dirname16, join as join46 } from "node:path";
var LOCAL_MANIFEST_PATH = join46(homedir23(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join46(homedir23(), ".claude", "hivemind", "local-mined.lock");
...
async function runMineLocalImpl(args) {
  ...
  const tmpDir = join47(homedir24(), ".claude", "hivemind", `mine-local-${Date.now()}`);
  ...
  const results = await parallelMap(picked, GATE_CONCURRENCY, async (s) => {
    ...
    const sessionTmp = join47(tmpDir, `s-${shortId}`);
    mkdirSync24(sessionTmp, { recursive: true });
    const verdictPath = join47(sessionTmp, "verdict.json");
    const prompt = buildSessionPrompt(tail, s, verdictPath);
    writeFileSync26(join47(sessionTmp, "prompt.txt"), prompt);
    const gate = await runGateViaStdin({ agent: gateAgent, bin: gateBin, prompt, timeoutMs: GATE_TIMEOUT_MS });
    try {
      writeFileSync26(join47(sessionTmp, "gate-stdout.txt"), gate.stdout);
      if (gate.stderr)
        writeFileSync26(join47(sessionTmp, "gate-stderr.txt"), gate.stderr);
    } catch {
    }
    if (gate.errored) {
    ...
    const verdictText = existsSync36(verdictPath) ? readFileSync33(verdictPath, "utf-8") : gate.stdout;
...
function showStatus() {
  ...
  for (const f of files) {
    try {
      const s = JSON.parse(readFileSync34(join48(dir, f), "utf-8"));
...
function setInstall(loc2) {
  ...
  const path = loc2 === "global" ? join48(homedir25(), ".claude", "skills") : "<cwd>/.claude/skills";
...
function promoteSkill(name, cwd) {
  ...
  const projectPath = join48(cwd, ".claude", "skills", name);
  const globalPath = join48(homedir25(), ".claude", "skills", name);
  if (!existsSync37(join48(projectPath, "SKILL.md"))) {
    console.error(`Skill '${name}' not found at ${projectPath}/SKILL.md`);
    process.exit(1);
  }
  if (existsSync37(join48(globalPath, "SKILL.md"))) {
...
async function pullSkills(args) {
  ...
  const dest = toRaw === "global" ? join48(homedir25(), ".claude", "skills") : join48(process.cwd(), ".claude", "skills");
...
async function pushSkills(args) {
  ...
  const src = fromRaw === "global" ? join48(homedir25(), ".claude", "skills") : join48(process.cwd(), ".claude", "skills");
...
import { existsSync as existsSync38, mkdirSync as mkdirSync26, readFileSync as readFileSync35, renameSync as renameSync13, writeFileSync as writeFileSync27 } from "node:fs";
import { homedir as homedir26 } from "node:os";
import { dirname as dirname19, join as join49 } from "node:path";
var HIVEMIND_DIR3 = join49(homedir26(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join49(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join49(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join49(HIVEMIND_DIR3, "pending-memory.lock");
...
function writePendingMemoryManifest(m, path = PENDING_MEMORY_MANIFEST_PATH) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync27(tmp, JSON.stringify(m, null, 2));
...
import { existsSync as existsSync40, mkdirSync as mkdirSync29, readFileSync as readFileSync38, writeFileSync as writeFileSync29, unlinkSync as unlinkSync14 } from "node:fs";
import { join as join56 } from "node:path";

// dist/src/hooks/spawn-wiki-worker.js
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { dirname as dirname21, join as join53 } from "node:path";
import { writeFileSync as writeFileSync28, mkdirSync as mkdirSync28 } from "node:fs";
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join50(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join52(homedir27(), ".claude", "local", cli);
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir28();
var wikiLogger = makeWikiLogger(join53(HOME3, ".claude", "hooks"));
...
import { openSync as openSync3, closeSync as closeSync3, writeSync as writeSync2, unlinkSync as unlinkSync13, existsSync as existsSync39, readFileSync as readFileSync37 } from "node:fs";
import { homedir as homedir29 } from "node:os";
import { join as join54 } from "node:path";
var SHARED_DAEMON_PATH2 = join54(homedir29(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync39(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync39(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    if (hello.daemonPath !== this.daemonEntry && !existsSync39(hello.daemonPath)) {
      _recycledStuckDaemon = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync37(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync39(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
    ...
    try {
      unlinkSync13(this.socketPath);
    } catch {
    }
    try {
      unlinkSync13(this.pidPath);
    } catch {
    }
  }
  /**
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync39(this.daemonEntry)) {
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync37(this.pidPath, "utf-8").trim();
...
function defaultResolveTransformers() {
  const sharedDir = join55(homedir30(), ".hivemind", "embed-deps");
  try {
...
function countLines(path) {
  try {
    const buf = readFileSync38(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join56(stagingDir, `${key}.md`);
  const embeddingPath = join56(stagingDir, `${key}.embedding.json`);
  if (!existsSync40(input.jsonlPath)) {
  ...
  if (existsSync40(summaryPath)) {
    try {
      unlinkSync14(summaryPath);
    } catch {
    }
  }
  ...
  const text = readFileSync38(summaryPath, "utf-8");
  if (!text.trim()) {
  ...
  if (!opts.skipEmbed) {
    try {
      ...
      if (vec) {
        writeFileSync29(embeddingPath, JSON.stringify(vec));
        embedded = true;
      }
    } catch {
    }
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync41, unlinkSync as unlinkSync15 } from "node:fs";
...
function releaseBackfillLock(lockPath2 = PENDING_MEMORY_LOCK_PATH) {
  if (process.env.HIVEMIND_BACKFILL_LOCK_OWNED !== "1")
    return;
  try {
    if (existsSync41(lockPath2))
      unlinkSync15(lockPath2);
  } catch {
  }
}
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync39(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync42(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync39(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
...
import { closeSync as closeSync5, existsSync as existsSync44, mkdirSync as mkdirSync31, openSync as openSync5, statSync as statSync9, unlinkSync as unlinkSync17 } from "node:fs";
...
import { closeSync as closeSync4, existsSync as existsSync43, mkdirSync as mkdirSync30, openSync as openSync4, readdirSync as readdirSync9, statSync as statSync8, unlinkSync as unlinkSync16 } from "node:fs";
import { homedir as homedir32 } from "node:os";
import { dirname as dirname22, join as join57 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var HOME4 = homedir32();
var HIVEMIND_DIR4 = join57(HOME4, ".claude", "hivemind");
var LOG_PATH = join57(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join57(HOME4, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname22(fileURLToPath3(import.meta.url));
    const cliPath = join57(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync43(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir33();
var HIVEMIND_DIR5 = join58(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join58(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync31(join58(HOME5, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync44(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync44(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
    ...
    removeLock: () => {
      try {
        unlinkSync17(PENDING_MEMORY_LOCK_PATH);
        return true;
      } catch {
        return false;
      }
...
import { closeSync as closeSync6, existsSync as existsSync45, mkdirSync as mkdirSync32, openSync as openSync6, readFileSync as readFileSync40, realpathSync, unlinkSync as unlinkSync18, writeSync as writeSync3 } from "node:fs";
import { homedir as homedir34 } from "node:os";
import { dirname as dirname23, join as join59, sep as sep2 } from "node:path";
...
function defaultLockPath() {
  return join59(homedir34(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync45(`${gitDir}${sep2}.git`)) {
...
function tryAcquireLock(path) {
  ...
  let holderPid = 0;
  try {
    holderPid = Number(readFileSync40(path, "utf-8").trim()) || 0;
  } catch {
    try {
      return claim();
    } catch {
...
import { existsSync as existsSync47, readFileSync as readFileSync41, readdirSync as readdirSync10, unlinkSync as unlinkSync19 } from "node:fs";
...
async function runAdvisor(manifestPath3 = LOCAL_MANIFEST_PATH) {
  ...
  if (!claudeBin || !existsSync46(claudeBin)) {
    return null;
  }
...
function claudeProjectsDir() {
  return join60(homedir35(), ".claude", "projects");
}
function manifestPath2() {
  return join60(homedir35(), ".claude", "hivemind", "local-mined.json");
}
...
function manifestIsTrulyEmpty() {
  ...
  if (!existsSync47(p))
    return false;
  try {
    const m = JSON.parse(readFileSync41(p, "utf-8"));
    return Array.isArray(m.entries) && m.entries.length === 0;
  } catch {
    return false;
  }
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync10(join60(projectsDir, sub));
    } catch {
      continue;
    }
...
function runInstallScan() {
  return new Promise((resolve7) => {
    ...
    if (!cliPath || !existsSync47(cliPath)) {
