...
import { readdirSync as readdirSync7, existsSync as existsSync30, readFileSync as readFileSync22, mkdirSync as mkdirSync13, renameSync as renameSync7 } from "node:fs";
...
function expandToCandidateFiles(snap, changedFiles) {
...
function resolveCliBin(cli, fallback) {
  ...
  try {
    const out = execFileSync5(isWin ? "where" : "which", [cli], { encoding: "utf-8" });
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join34(homedir19(), ".claude", "local", cli);
...
function detectHostAgent2(resolve3 = tryResolveCliBin) {
  for (const name of ["claude", "codex", "pi", "cursor"]) {
    const spec = REGISTRY[name]({});
    if (resolve3(spec.bin) !== null)
      return name;
  }
...
function resolveDocLlmSpec(env = process.env) {
  ...
  const agent = (env.HIVEMIND_DOCS_LLM_AGENT ?? detectHostAgent2()).toLowerCase();
...
function runHostPrompt(spec, bin, prompt, timeoutMs = 12e4) {
  ...
  const out = execFileSync6(inv.file, inv.args, {
    ...inv.options,
    encoding: "utf-8",
    timeout: timeoutMs,
    env: { ...process.env, HIVEMIND_WIKI_WORKER: "1", HIVEMIND_CAPTURE: "false" }
...
async function updateWikiPage(args) {
  ...
  try {
    ...
    const res = await editDoc(args.query, args.tableName, {
...
async function runWikiRefreshCycle(args) {
  ...
  if (lastSha !== "") {
    ...
    if (out !== null) {
      ...
      log7(`diff ${lastSha}..HEAD unavailable \u2014 full-candidate cycle`);
    }
  } else {
  ...
  for (const d of await listDocs(args.query, args.tableName, { project: args.project, status: "active", limit: 1e5 })) {
  ...
  if (failures > 0) {
    log7(`${failures} page(s) failed \u2014 sha NOT advanced, next turn redoes the window`);
...
function readPullManifest(repoRoot) {
  try {
    const raw = JSON.parse(readFileSync24(join36(repoRoot, PULL_MANIFEST_DIR, PULL_MANIFEST_FILE), "utf-8"));
...
function writePullManifest(repoRoot, manifest) {
  ...
  writeFileSync19(join36(dir, PULL_MANIFEST_FILE), JSON.stringify(manifest, null, 2) + "\n");
}
...
var SHARED_DAEMON_PATH2 = join37(homedir20(), ".hivemind", "embed-deps", "embed-daemon.js");
...
function defaultResolveTransformers() {
  const sharedDir = join38(homedir21(), ".hivemind", "embed-deps");
  try {
...
function gitHeadOf(cwd) {
  try {
    return execFileSync7("git", ["-C", cwd, "rev-parse", "HEAD"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim() || null;
  } catch {
    return null;
  }
}
...
async function runDocsCommand(args) {
  ...
  if (sub === "list") {
    ...
    let rows = [];
    try {
      rows = allView ? await listDocs(query, tableName, { status, limit }) : await listDocs(query, tableName, { status, projectOrLegacy: explicitProject !== void 0 ? resolveProjectArg(explicitProject) : headerProject, limit });
    } catch (err) {
      if (!isMissingTableError(err.message))
        throw err;
    }
...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "wiki-refresh") {
    ...
    const git = (gitArgs) => {
      try {
        return execFileSync7("git", ["-C", cwd, ...gitArgs], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
      } catch {
        return null;
      }
    };
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-JLO72WBE.js");
  ...
  if (sub === "wiki") {
    ...
    let existingDocs = [];
    try {
      existingDocs = await listDocs(query, tableName, { status: "all", limit: 1e6 });
    } catch (err) {
      if (!isMissingTableError(err.message))
        throw err;
    }
...
import { existsSync as existsSync34, mkdirSync as mkdirSync15, readFileSync as readFileSync27, renameSync as renameSync8, writeFileSync as writeFileSync20 } from "node:fs";
import { homedir as homedir22 } from "node:os";
import { dirname as dirname13, join as join40 } from "node:path";
var HIVEMIND_DIR3 = join40(homedir22(), ".claude", "hivemind");
...
import { existsSync as existsSync35, mkdirSync as mkdirSync18, readFileSync as readFileSync29, writeFileSync as writeFileSync22, unlinkSync as unlinkSync11 } from "node:fs";
...
import { homedir as homedir23, tmpdir } from "node:os";
...
async function stageSession(input, opts) {
  ...
  if (!existsSync35(input.jsonlPath)) {
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync36, unlinkSync as unlinkSync12 } from "node:fs";
...
import { existsSync as existsSync37, readFileSync as readFileSync30 } from "node:fs";
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync37(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
...
import { closeSync as closeSync4, existsSync as existsSync39, mkdirSync as mkdirSync20, openSync as openSync4, statSync as statSync8, unlinkSync as unlinkSync14 } from "node:fs";
...
import { closeSync as closeSync3, existsSync as existsSync38, mkdirSync as mkdirSync19, openSync as openSync3, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync13 } from "node:fs";
...
function findBundledCliPath() {
  try {
    ...
    return existsSync38(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
function findHivemindLauncher() {
  ...
  try {
    ...
    const out = execFileSync8(lookup, ["hivemind"], {
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "ignore"]
    });
    const bin = out.trim();
...
function hasLocalClaudeSessions() {
  if (!existsSync38(CLAUDE_PROJECTS_DIR))
    return false;
  let subdirs;
  try {
...
function maybeAutoMineLocal(opts = {}) {
  if (existsSync38(LOCAL_MANIFEST_PATH))
    return { triggered: false, reason: "manifest-exists" };
  if (existsSync38(LOCAL_MINE_LOCK_PATH)) {
    let stale = false;
    try {
  ...
  try {
    mkdirSync19(join45(HOME4, ".claude", "hooks"), { recursive: true });
...
function realSpawn() {
  ...
  if (!launcher)
    return false;
  mkdirSync20(join46(HOME5, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync39(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync39(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
...
import { closeSync as closeSync5, existsSync as existsSync40, mkdirSync as mkdirSync21, openSync as openSync5, readFileSync as readFileSync31, realpathSync, unlinkSync as unlinkSync15, writeSync as writeSync3 } from "node:fs";
...
function defaultLockPath() {
  return join47(homedir27(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync40(`${gitDir}${sep2}.git`)) {
...
var defaultSpawn = (cmd, args) => {
  execFileSync9(cmd, args, { stdio: "inherit" });
};
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-JLO72WBE.js"));
    } catch (err) {
