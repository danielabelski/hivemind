...
import { existsSync as existsSync15, mkdirSync as mkdirSync7, readFileSync as readFileSync15, writeFileSync as writeFileSync12 } from "node:fs";
import { join as join20 } from "node:path";
import { tmpdir } from "node:os";
function getIndexMarkerDir() {
  return process.env.HIVEMIND_INDEX_MARKER_DIR ?? join20(tmpdir(), "hivemind-deeplake-indexes");
}
function buildIndexMarkerPath(workspaceId, orgId, table, suffix) {
  const markerKey = [workspaceId, orgId, table, suffix].join("__").replace(/[^a-zA-Z0-9_.-]/g, "_");
  return join20(getIndexMarkerDir(), `${markerKey}.json`);
}
function hasFreshIndexMarker(markerPath) {
  if (!existsSync15(markerPath))
    return false;
  try {
    const raw = JSON.parse(readFileSync15(markerPath, "utf-8"));
...
function writeIndexMarker(markerPath) {
  ...
  writeFileSync12(markerPath, JSON.stringify({ updatedAt: (/* @__PURE__ */ new Date()).toISOString() }), "utf-8");
}
...
var AGENTS_MD = join4(CODEX_HOME, "AGENTS.md");
...
import { existsSync as existsSync9, readFileSync as readFileSync7, writeFileSync as writeFileSync6, unlinkSync as unlinkSync5 } from "node:fs";
import { join as join10 } from "node:path";
var CONFIG_DIR = claudeDesktopConfigDir();
var CONFIG_PATH2 = join10(CONFIG_DIR, "claude_desktop_config.json");
var SERVER_KEY2 = "hivemind";
function readConfig2() {
  if (!existsSync9(CONFIG_PATH2))
    return {};
  const txt = readFileSync7(CONFIG_PATH2, "utf-8").trim();
  if (!txt)
    return {};
  let parsed;
  try {
...
import { existsSync as existsSync10, writeFileSync as writeFileSync7, rmSync as rmSync5, readFileSync as readFileSync8, copyFileSync as copyFileSync2 } from "node:fs";
import { join as join11 } from "node:path";
var PI_AGENT_DIR = join11(HOME, ".pi", "agent");
var AGENTS_MD2 = join11(PI_AGENT_DIR, "AGENTS.md");
var LEGACY_SKILL_DIR = join11(PI_AGENT_DIR, "skills", "hivemind-memory");
var EXTENSIONS_DIR = join11(PI_AGENT_DIR, "extensions");
var EXTENSION_PATH = join11(EXTENSIONS_DIR, "hivemind.ts");
var VERSION_DIR = join11(PI_AGENT_DIR, ".hivemind");
var WIKI_WORKER_DIR = join11(PI_AGENT_DIR, "hivemind");
var WIKI_WORKER_PATH = join11(WIKI_WORKER_DIR, "wiki-worker.js");
var SKILLIFY_WORKER_PATH = join11(WIKI_WORKER_DIR, "skillify-worker.js");
var AUTOPULL_WORKER_PATH = join11(WIKI_WORKER_DIR, "autopull-worker.js");
var SKILLOPT_WORKER_PATH = join11(WIKI_WORKER_DIR, "skillopt-worker.js");
...
function installPi() {
  ...
  if (existsSync10(LEGACY_SKILL_DIR)) {
    rmSync5(LEGACY_SKILL_DIR, { recursive: true, force: true });
  }
  const prior = existsSync10(AGENTS_MD2) ? readFileSync8(AGENTS_MD2, "utf-8") : null;
  const next = upsertHivemindBlock(prior);
  writeFileSync7(AGENTS_MD2, next);
  const srcExtension = join11(pkgRoot(), "harnesses", "pi", "extension-source", "hivemind.ts");
  if (!existsSync10(srcExtension)) {
  ...
  const srcWorker = join11(pkgRoot(), "harnesses", "pi", "bundle", "wiki-worker.js");
  if (existsSync10(srcWorker)) {
    ensureDir(WIKI_WORKER_DIR);
    copyFileSync2(srcWorker, WIKI_WORKER_PATH);
  }
  const srcSkillifyWorker = join11(pkgRoot(), "harnesses", "pi", "bundle", "skillify-worker.js");
  if (existsSync10(srcSkillifyWorker)) {
    ensureDir(WIKI_WORKER_DIR);
    copyFileSync2(srcSkillifyWorker, SKILLIFY_WORKER_PATH);
  }
  const srcAutopullWorker = join11(pkgRoot(), "harnesses", "pi", "bundle", "autopull-worker.js");
  if (existsSync10(srcAutopullWorker)) {
    ensureDir(WIKI_WORKER_DIR);
    copyFileSync2(srcAutopullWorker, AUTOPULL_WORKER_PATH);
  }
  const srcSkilloptWorker = join11(pkgRoot(), "harnesses", "pi", "bundle", "skillopt-worker.js");
  if (existsSync10(srcSkilloptWorker)) {
    ensureDir(WIKI_WORKER_DIR);
...
function uninstallPi() {
  ...
  if (existsSync10(EXTENSION_PATH)) {
    rmSync5(EXTENSION_PATH, { force: true });
    log(`  pi             removed extension ${EXTENSION_PATH}`);
  }
  if (existsSync10(WIKI_WORKER_DIR)) {
    rmSync5(WIKI_WORKER_DIR, { recursive: true, force: true });
    log(`  pi             removed wiki-worker dir ${WIKI_WORKER_DIR}`);
  }
  if (existsSync10(AGENTS_MD2)) {
    const prior = readFileSync8(AGENTS_MD2, "utf-8");
    const stripped = stripHivemindBlock(prior);
    if (stripped.trim().length === 0) {
      rmSync5(AGENTS_MD2, { force: true });
      log(`  pi             removed empty ${AGENTS_MD2}`);
    } else {
      writeFileSync7(AGENTS_MD2, stripped);
      log(`  pi             stripped hivemind block from ${AGENTS_MD2}`);
    }
  }
  if (existsSync10(VERSION_DIR)) {
    rmSync5(VERSION_DIR, { recursive: true, force: true });
  }
}

// dist/src/cli/embeddings.js
import { copyFileSync as copyFileSync3, chmodSync, existsSync as existsSync12, lstatSync as lstatSync5, readdirSync, readFileSync as readFileSync10, readlinkSync, rmSync as rmSync6, statSync, unlinkSync as unlinkSync6 } from "node:fs";
...
import { existsSync as existsSync11, mkdirSync as mkdirSync2, readFileSync as readFileSync9, renameSync as renameSync2, writeFileSync as writeFileSync8 } from "node:fs";
import { homedir as homedir4 } from "node:os";
import { dirname as dirname2, join as join12 } from "node:path";
var _configPath = () => process.env.HIVEMIND_CONFIG_PATH ?? join12(homedir4(), ".deeplake", "config.json");
var _cache = null;
var _migrated = false;
...
// dist/src/cli/embeddings.js
var SHARED_DIR = join13(HOME, ".hivemind", "embed-deps");
var SHARED_NODE_MODULES = join13(SHARED_DIR, "node_modules");
var SHARED_DAEMON_PATH = join13(SHARED_DIR, "embed-daemon.js");
var TRANSFORMERS_PKG = "@huggingface/transformers";
var TRANSFORMERS_RANGE = "^3.0.0";
function findHivemindInstalls(home = HOME) {
  const out = [];
  const fixed = [
    { id: "codex", pluginDir: join13(home, ".codex", "hivemind") },
    { id: "cursor", pluginDir: join13(home, ".cursor", "hivemind") },
    { id: "hermes", pluginDir: join13(home, ".hermes", "hivemind") }
  ];
  for (const inst of fixed) {
    if (existsSync12(join13(inst.pluginDir, "bundle")))
      out.push(inst);
  }
  const ccCache = join13(home, ".claude", "plugins", "cache", "hivemind", "hivemind");
  if (existsSync12(ccCache)) {
    let entries = [];
    try {
    ...
    for (const ver of entries) {
      ...
      const candidates = [join13(dir, "bundle"), join13(dir, "harnesses", "claude-code", "bundle")];
      if (candidates.some((p) => existsSync12(p))) {
...
function isSharedDepsInstalled(sharedNodeModules = SHARED_NODE_MODULES) {
  return existsSync12(join13(sharedNodeModules, TRANSFORMERS_PKG));
}
...
function linkStateFor(install, sharedNodeModules = SHARED_NODE_MODULES) {
  const link = join13(install.pluginDir, "node_modules");
  if (!existsSync12(link) && !isSymbolicLink(link))
...
function ensureSharedDeps() {
  if (!isSharedDepsInstalled()) {
    ...
    writeJson(join13(SHARED_DIR, "package.json"), {
  ...
  ensureDir(SHARED_DIR);
  const src = join13(pkgRoot(), "embeddings", "embed-daemon.js");
  if (existsSync12(src)) {
...
function uninstallEmbeddings(opts) {
  const installs = findHivemindInstalls();
  for (const inst of installs) {
    const link = join13(inst.pluginDir, "node_modules");
    if (isSymlinkToSharedDeps(link, SHARED_NODE_MODULES)) {
      unlinkSync6(link);
      log(`  Embeddings     unlinked ${inst.id}`);
    }
  }
  if (opts?.prune && existsSync12(SHARED_DIR)) {
...
function killEmbedDaemon(socketDir) {
  ...
  let pid = null;
  try {
    pid = Number.parseInt(readFileSync10(pidPath, "utf-8").trim(), 10);
  } catch {
  }
...
function statusEmbeddings() {
  ...
  log(`Daemon:        ${existsSync12(SHARED_DAEMON_PATH) ? SHARED_DAEMON_PATH : "(not present)"}`);
  if (!enabled) {
    log("");
...
import { readFileSync as readFileSync11, writeFileSync as writeFileSync9, mkdirSync as mkdirSync3 } from "node:fs";
import { join as join14 } from "node:path";
import { homedir as homedir5 } from "node:os";
import { randomUUID } from "node:crypto";
function configDir() {
  return join14(homedir5(), ".deeplake");
}
function installIDPath() {
  return join14(configDir(), "install-id");
}
var UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function getOrCreateInstallID() {
  try {
    const value = readFileSync11(installIDPath(), "utf-8").trim();
  ...
  try {
    ...
    writeFileSync9(installIDPath(), id, { mode: 384 });
    return id;
  } catch {
    return "";
  }
...
import { readFileSync as readFileSync12, writeFileSync as writeFileSync10, mkdirSync as mkdirSync4, unlinkSync as unlinkSync7, renameSync as renameSync3 } from "node:fs";
import { join as join15 } from "node:path";
import { homedir as homedir6 } from "node:os";
function configDir2() {
  return join15(homedir6(), ".deeplake");
}
function credsPath() {
  return join15(configDir2(), "credentials.json");
}
...
var CREDS_PATH = join16(HOME, ".deeplake", "credentials.json");
function isLoggedIn() {
  return existsSync13(CREDS_PATH) && loadCredentials() !== null;
}
...
function loadConfig() {
  const home = homedir7();
  const credPath = join17(home, ".deeplake", "credentials.json");
  let creds = null;
  if (existsSync14(credPath)) {
    try {
      creds = JSON.parse(readFileSync13(credPath, "utf-8"));
    } catch {
      return null;
    }
  }
  ...
  return {
    ...
    memoryPath: process.env.HIVEMIND_MEMORY_PATH ?? join17(home, ".deeplake", "memory")
  };
}
...
var LOG = join18(homedir8(), ".deeplake", "hook-debug.log");
function isDebug() {
...
import { readFileSync as readFileSync14, writeFileSync as writeFileSync11, mkdirSync as mkdirSync6, openSync, closeSync, unlinkSync as unlinkSync8, statSync as statSync2 } from "node:fs";
...
function queuePath() {
  return join19(homedir9(), ".deeplake", "notifications-queue.json");
}
function lockPath() {
...
function readQueue() {
  try {
    const raw = readFileSync14(queuePath(), "utf-8");
    const parsed = JSON.parse(raw);
...
function writeQueue(q) {
  ...
  const home = resolve2(homedir9());
  if (!_isQueuePathInsideHome(path, home)) {
    throw new Error(`notifications-queue write blocked: ${path} is outside ${home}`);
  }
  mkdirSync6(join19(home, ".deeplake"), { recursive: true, mode: 448 });
  const tmp = `${path}.${process.pid}.tmp`;
  writeFileSync11(tmp, JSON.stringify(q, null, 2), { mode: 384 });
  renameAtomic(tmp, path);
}
async function withQueueLock(fn) {
  const path = lockPath();
  mkdirSync6(join19(homedir9(), ".deeplake"), { recursive: true, mode: 448 });
  let fd = null;
...
import { readFileSync as readFileSync22, readdirSync as readdirSync2 } from "node:fs";
import { join as join29, relative as relative2, resolve as resolve5, sep } from "node:path";
import { createHash as createHash6 } from "node:crypto";

// dist/src/graph/cache.js
import { createHash } from "node:crypto";
import { existsSync as existsSync16, mkdirSync as mkdirSync8, readFileSync as readFileSync16, renameSync as renameSync4, writeFileSync as writeFileSync13 } from "node:fs";
...
function cacheDir(baseDir) {
  return join21(baseDir, ".cache");
}
function cachePath(baseDir, contentSha256) {
  return join21(cacheDir(baseDir), `${contentSha256}.json`);
}
...
function writeCache(baseDir, contentSha256, extraction) {
  ...
  try {
    ...
    writeFileSync13(tmp, JSON.stringify(entry));
...
import { existsSync as existsSync19, mkdirSync as mkdirSync12, renameSync as renameSync7, writeFileSync as writeFileSync16 } from "node:fs";
...
import { existsSync as existsSync17, mkdirSync as mkdirSync9, readFileSync as readFileSync17, renameSync as renameSync5, writeFileSync as writeFileSync14 } from "node:fs";
import { dirname as dirname5, join as join22 } from "node:path";
function lastBuildPath(baseDir, worktreeId) {
  if (worktreeId !== void 0) {
    return join22(baseDir, "worktrees", worktreeId, ".last-build.json");
  }
  return join22(baseDir, ".last-build.json");
}
...
import { appendFileSync as appendFileSync2, existsSync as existsSync18, mkdirSync as mkdirSync10, readFileSync as readFileSync18 } from "node:fs";
import { dirname as dirname6, join as join23 } from "node:path";
function historyPath(baseDir) {
  return join23(baseDir, "history.jsonl");
}
...
import { mkdirSync as mkdirSync11, renameSync as renameSync6, writeFileSync as writeFileSync15 } from "node:fs";
...
// dist/src/graph/snapshot.js
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join24(homedir10(), ".hivemind", "graphs");
}
function repoDir(repoKey) {
  return join24(graphsRoot(), repoKey);
}
...
function writeSnapshot(snapshot, baseDir, trigger = "unknown", worktreeId) {
  ...
  const snapshotPath = join24(snapshotsDir, `${fileBase}.json`);
  const canonical = canonicalSnapshot(snapshot);
  writeFileAtomic(snapshotPath, canonical);
  const worktreeRoot = worktreeId !== void 0 ? join24(baseDir, "worktrees", worktreeId) : baseDir;
  let latestCommitPath = null;
  if (commitSha !== null) {
    latestCommitPath = join24(worktreeRoot, "latest-commit.txt");
...
async function pullSnapshot(cwd, deps = {}) {
  ...
  const snapshotsDir = join25(baseDir, "snapshots");
  const snapshotPath = join25(snapshotsDir, `${head}.json`);
  const worktreeRoot = join25(baseDir, "worktrees", worktreeId);
  try {
    writeFileAtomic2(snapshotPath, cloudPayload);
    writeFileAtomic2(join25(worktreeRoot, "latest-commit.txt"), `${head}
`);
    writeLastBuild(baseDir, {
      ts: cloudTs,
      commit_sha: head,
...
function loadSnapshotByCommit(baseDir, commitSha) {
  if (!/^[0-9a-f]{4,64}$/i.test(commitSha))
    return null;
  const path = join26(baseDir, "snapshots", `${commitSha}.json`);
...
import { mkdirSync as mkdirSync13, readFileSync as readFileSync20, writeFileSync as writeFileSync17 } from "node:fs";
...
function loadGraphIgnore(deeplakeDir = join27(homedir11(), ".deeplake")) {
  const path = join27(deeplakeDir, FILE_NAME);
  try {
  ...
  try {
    ...
    writeFileSync17(path, JSON.stringify(defaultConfigObject(), null, 2) + "\n", { flag: "wx" });
  } catch {
  }
...
import { chmodSync as chmodSync2, existsSync as existsSync21, mkdirSync as mkdirSync14, readFileSync as readFileSync21, unlinkSync as unlinkSync9, writeFileSync as writeFileSync18 } from "node:fs";
import { dirname as dirname9, join as join28, resolve as resolve4 } from "node:path";
...
function gitHooksDir(cwd) {
  ...
  if (configured !== null) {
    ...
    return top !== null ? resolve4(top, configured) : resolve4(cwd, configured);
  }
  try {
...
function postCommitHookPath(cwd) {
  ...
  return hooksDir === null ? null : join28(hooksDir, "post-commit");
}
function installPostCommitHook(cwd, opts = {}) {
  ...
  writeFileSync18(path, buildHookFile(hivemindPath), { mode: 493 });
  try {
    chmodSync2(path, 493);
  } catch {
  }
...
async function runBuildCommand(args) {
  ...
  for (const abs of sourceFiles) {
    ...
    try {
      const content = readFileSync22(abs, "utf8");
...
import { dirname as dirname13, join as join37, resolve as resolve6 } from "node:path";

// dist/src/dashboard/data.js
import { existsSync as existsSync26, readFileSync as readFileSync26, readdirSync as readdirSync4, statSync as statSync3 } from "node:fs";
import { homedir as homedir15 } from "node:os";
import { join as join35 } from "node:path";

// dist/src/notifications/sources/org-stats.js
import { existsSync as existsSync22, mkdirSync as mkdirSync15, readFileSync as readFileSync23, writeFileSync as writeFileSync19 } from "node:fs";
...
function cacheFilePath() {
  return join30(homedir12(), ".deeplake", "hivemind-stats-cache.json");
}
...
function readCache2(scopeKey) {
  if (!existsSync22(cacheFilePath()))
    return {};
  try {
    const parsed = JSON.parse(readFileSync23(cacheFilePath(), "utf-8"));
    if (!parsed || typeof parsed !== "object")
      return {};
...
function writeCache2(scopeKey, data) {
  try {
    ...
    writeFileSync19(cacheFilePath(), JSON.stringify(body), "utf-8");
  } catch (e) {
...
import { appendFileSync as appendFileSync3, existsSync as existsSync23, mkdirSync as mkdirSync16, readFileSync as readFileSync24, readdirSync as readdirSync3 } from "node:fs";
...
function statsFilePath() {
  return join31(homedir13(), ".deeplake", "usage-stats.jsonl");
}
function readUsageRecords() {
  try {
    if (!existsSync23(statsFilePath()))
      return [];
    const raw = readFileSync24(statsFilePath(), "utf-8");
    const out = [];
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join31(homedir13(), ".claude", "skills");
  if (!existsSync23(dir))
    return 0;
...
import { readFileSync as readFileSync25, writeFileSync as writeFileSync20, writeSync, mkdirSync as mkdirSync17, renameSync as renameSync9, rmdirSync, existsSync as existsSync25, lstatSync as lstatSync6, unlinkSync as unlinkSync10, openSync as openSync2, closeSync as closeSync2 } from "node:fs";
import { join as join34 } from "node:path";

// dist/src/skillify/legacy-migration.js
import { existsSync as existsSync24, renameSync as renameSync8 } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join32(homedir14(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join33(dirname12(current), "skilify");
...
function graphsRoot2() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join35(homedir15(), ".hivemind", "graphs");
}
...
function resolveSnapshot(repoDir2) {
  const snapshotsDir = join35(repoDir2, "snapshots");
  if (!existsSync26(snapshotsDir))
    return null;
  let snapshotPath = null;
  const pointer = join35(repoDir2, "latest-commit.txt");
  if (existsSync26(pointer)) {
    try {
      const sha = readFileSync26(pointer, "utf-8").trim();
      if (sha) {
        const candidate = join35(snapshotsDir, `${sha}.json`);
  ...
  if (!snapshotPath) {
    try {
      const candidates = readdirSync4(snapshotsDir).filter((name) => name.endsWith(".json")).map((name) => {
        const full = join35(snapshotsDir, name);
  ...
  let raw;
  try {
    raw = readFileSync26(snapshotPath, "utf-8");
  } catch (e) {
...
async function loadDashboardData(opts = {}) {
  ...
  const repoDir2 = join35(opts.graphsHome ?? graphsRoot2(), repoKey);
...
import { delimiter, join as join36 } from "node:path";
...
function findBinaryOnPath(name) {
  ...
  for (const dir of PATH.split(delimiter)) {
    ...
    for (const ext of exts) {
      const candidate = join36(dir, name + ext);
      try {
...
function makeHandle(server, host, port) {
  let resolveStopped;
  const stopped = new Promise((resolve7) => {
    resolveStopped = resolve7;
  });
...
function defaultDashboardOutPath(repoKey) {
  return join37(homedir16(), ".hivemind", "dashboards", repoKey, "index.html");
}
...
`);
  ...
  const absOut = resolve6(finalOut);
  try {
    mkdirSync18(dirname13(absOut), { recursive: true });
    writeFileSync21(absOut, html, "utf-8");
  } catch (e) {
...
import { readdirSync as readdirSync8, existsSync as existsSync38, readFileSync as readFileSync35, mkdirSync as mkdirSync25, renameSync as renameSync12 } from "node:fs";
import { homedir as homedir25 } from "node:os";
import { dirname as dirname18, join as join49 } from "node:path";

// dist/src/skillify/scope-config.js
import { existsSync as existsSync27, mkdirSync as mkdirSync19, readFileSync as readFileSync27, writeFileSync as writeFileSync22 } from "node:fs";
import { join as join38 } from "node:path";
function configPath() {
  return join38(getStateDir(), "config.json");
}
var DEFAULT = { scope: "me", team: [], install: "project" };
function loadScopeConfig() {
  migrateLegacyStateDir();
  const CONFIG_PATH3 = configPath();
  if (!existsSync27(CONFIG_PATH3))
    return DEFAULT;
  try {
    const raw = JSON.parse(readFileSync27(CONFIG_PATH3, "utf-8"));
...
function saveScopeConfig(cfg) {
  ...
  writeFileSync22(configPath(), JSON.stringify(cfg, null, 2));
}

// dist/src/skillify/pull.js
import { existsSync as existsSync31, readFileSync as readFileSync30, writeFileSync as writeFileSync25, mkdirSync as mkdirSync22, renameSync as renameSync11, lstatSync as lstatSync8, readlinkSync as readlinkSync2, symlinkSync as symlinkSync2, unlinkSync as unlinkSync12 } from "node:fs";
import { homedir as homedir19 } from "node:os";
import { dirname as dirname15, join as join42 } from "node:path";

// dist/src/skillify/skill-writer.js
import { existsSync as existsSync28, mkdirSync as mkdirSync20, readFileSync as readFileSync28, readdirSync as readdirSync5, statSync as statSync5, writeFileSync as writeFileSync23 } from "node:fs";
...
function skillDir(skillsRoot, name) {
  return join39(skillsRoot, name);
}
function skillPath(skillsRoot, name) {
  return join39(skillDir(skillsRoot, name), "SKILL.md");
}
...
function listSkills(skillsRoot) {
  if (!existsSync28(skillsRoot))
    return [];
  const out = [];
  for (const name of readdirSync5(skillsRoot)) {
    const skillFile = join39(skillsRoot, name, "SKILL.md");
    if (existsSync28(skillFile) && statSync5(skillFile).isFile()) {
      out.push({ name, body: readFileSync28(skillFile, "utf-8") });
    }
  }
  return out;
}
function resolveSkillsRoot(install, cwd) {
  if (install === "global") {
    return join39(homedir17(), ".claude", "skills");
  }
  return join39(cwd, ".claude", "skills");
}

// dist/src/skillify/manifest.js
import { existsSync as existsSync29, lstatSync as lstatSync7, mkdirSync as mkdirSync21, readFileSync as readFileSync29, renameSync as renameSync10, unlinkSync as unlinkSync11, writeFileSync as writeFileSync24 } from "node:fs";
import { dirname as dirname14, join as join40 } from "node:path";
function emptyManifest() {
  return { version: 1, entries: [] };
}
function manifestPath() {
  return join40(getStateDir(), "pulled.json");
}
...
function saveManifest(m, path = manifestPath()) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync24(tmp, JSON.stringify(m, null, 2) + "\n", { mode: 384 });
...
function pruneOrphanedEntries(path = manifestPath()) {
  ...
  for (const e of m.entries) {
    if (existsSync29(join40(e.installRoot, e.dirName))) {
      live.push(e);
      continue;
    }
...
function resolveDetected(home) {
  const out = [];
  const codexInstalled = existsSync30(join41(home, ".codex"));
  const piInstalled = existsSync30(join41(home, ".pi", "agent"));
  const hermesInstalled = existsSync30(join41(home, ".hermes"));
  if (codexInstalled || piInstalled) {
    out.push(join41(home, ".agents", "skills"));
  }
  if (hermesInstalled) {
    out.push(join41(home, ".hermes", "skills"));
  }
  if (piInstalled) {
    out.push(join41(home, ".pi", "agent", "skills"));
  }
  return out;
}
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join42(homedir19(), ".claude", "skills");
  if (!cwd)
...
function fanOutSymlinks(canonicalDir, dirName, agentRoots) {
  ...
  for (const root of agentRoots) {
    const link = join42(root, dirName);
    let existing;
    try {
...
function backfillSymlinks(installRoot) {
  ...
  for (const entry of entries) {
    const canonical = join42(entry.installRoot, entry.dirName);
    if (!existsSync31(canonical))
      continue;
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    const skillDir2 = join42(root, dirName);
    const skillFile = join42(skillDir2, "SKILL.md");
    ...
    if (action === "wrote") {
      ...
      writeFileSync25(skillFile, renderSkillFile(row));
...
import { existsSync as existsSync32, readFileSync as readFileSync31 } from "node:fs";
...
function readLocalSkill(skillsRoot, name) {
  assertValidSkillName(name);
  const path = join43(skillsRoot, name, "SKILL.md");
  if (!existsSync32(path)) {
    throw new Error(`skill '${name}' not found at ${path}`);
  }
  const parsed = parseFrontmatter(readFileSync31(path, "utf-8"));
  if (!parsed) {
...
async function runPush(args) {
  ...
  const localPath = join43(skillsRoot, args.skillName, "SKILL.md");
...
import { existsSync as existsSync33, readdirSync as readdirSync6, rmSync as rmSync7, statSync as statSync6 } from "node:fs";
import { homedir as homedir20 } from "node:os";
import { join as join44 } from "node:path";
function resolveUnpullRoot(install, cwd) {
  if (install === "global")
    return join44(homedir20(), ".claude", "skills");
  if (!cwd)
    throw new Error("cwd required when install === 'project'");
  return join44(cwd, ".claude", "skills");
}
function runUnpull(opts) {
  ...
  for (const entry of entries) {
    summary.scanned++;
    const path = join44(root, entry.dirName);
  ...
  if (existsSync33(root) && (opts.all || opts.legacyCleanup)) {
    ...
    for (const dirName of readdirSync6(root)) {
      if (manifestDirNames.has(dirName))
        continue;
      const path = join44(root, dirName);
      let st;
      try {
        st = statSync6(path);
      } catch {
...
import { existsSync as existsSync37, mkdirSync as mkdirSync24, readFileSync as readFileSync34, writeFileSync as writeFileSync27 } from "node:fs";
import { homedir as homedir24 } from "node:os";
import { basename as basename2, dirname as dirname17, join as join48 } from "node:path";

// dist/src/skillify/local-source.js
import { readdirSync as readdirSync7, readFileSync as readFileSync32, existsSync as existsSync34, statSync as statSync7 } from "node:fs";
...
function detectInstalledAgents() {
  const installs = [];
  const claudeRoot = join45(HOME2, ".claude", "projects");
  if (existsSync34(claudeRoot)) {
    installs.push({
      agent: "claude_code",
  ...
  const codexRoot = join45(HOME2, ".codex", "sessions");
  if (existsSync34(codexRoot)) {
    installs.push({
      agent: "codex",
...
function collectJsonlRecursive(out, agent, dir, inCwd) {
  ...
  for (const e of entries) {
    if (e.isDirectory())
      collectJsonlRecursive(out, agent, join45(dir, e.name), inCwd);
    else if (e.isFile())
...
function listLocalSessions(installs, cwd) {
  ...
  for (const install of installs) {
    ...
    let top;
    try {
      top = readdirSync7(install.sessionRoot, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const entry of top) {
      ...
      if (entry.isDirectory()) {
        collectJsonlRecursive(out, install.agent, join45(install.sessionRoot, entry.name), inCwd);
  ...
  switch (agent) {
    ...
    case "claude_code":
      return firstExistingPath([
        join46(home, ".claude", "local", "claude"),
        "/usr/local/bin/claude",
        "/usr/bin/claude",
        join46(home, ".npm-global", "bin", "claude"),
        join46(home, ".local", "bin", "claude"),
    ...
    case "codex":
      return firstExistingPath([
        "/usr/local/bin/codex",
        "/usr/bin/codex",
        join46(home, ".npm-global", "bin", "codex"),
        join46(home, ".local", "bin", "codex"),
    ...
    case "cursor":
      return firstExistingPath([
        ...
        join46(home, ".npm-global", "bin", "cursor-agent"),
        join46(home, ".local", "bin", "cursor-agent"),
        "/opt/homebrew/bin/cursor-agent"
      ]) ?? "/usr/local/bin/cursor-agent";
    case "hermes":
      return firstExistingPath([
        join46(home, ".local", "bin", "hermes"),
        "/usr/local/bin/hermes",
        "/usr/bin/hermes",
        join46(home, ".npm-global", "bin", "hermes"),
        "/opt/homebrew/bin/hermes"
      ]) ?? join46(home, ".local", "bin", "hermes");
    case "pi":
      return firstExistingPath([
        join46(home, ".local", "bin", "pi"),
        "/usr/local/bin/pi",
        "/usr/bin/pi",
        join46(home, ".npm-global", "bin", "pi"),
        "/opt/homebrew/bin/pi"
      ]) ?? join46(home, ".local", "bin", "pi");
  }
}
...
import { existsSync as existsSync36, mkdirSync as mkdirSync23, readFileSync as readFileSync33, writeFileSync as writeFileSync26 } from "node:fs";
import { homedir as homedir23 } from "node:os";
import { dirname as dirname16, join as join47 } from "node:path";
var LOCAL_MANIFEST_PATH = join47(homedir23(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join47(homedir23(), ".claude", "hivemind", "local-mined.lock");
...
function writeLocalManifest(m, path = LOCAL_MANIFEST_PATH) {
  ...
  writeFileSync26(path, JSON.stringify(m, null, 2));
}
...
// dist/src/commands/mine-local.js
import { unlinkSync as unlinkSync13 } from "node:fs";
var EPSILON = 0.3;
var DEFAULT_N = 8;
...
async function runMineLocalImpl(args) {
  ...
  const tmpDir = join48(homedir24(), ".claude", "hivemind", `mine-local-${Date.now()}`);
  ...
  const results = await parallelMap(picked, GATE_CONCURRENCY, async (s) => {
    ...
    const sessionTmp = join48(tmpDir, `s-${shortId}`);
    mkdirSync24(sessionTmp, { recursive: true });
    const verdictPath = join48(sessionTmp, "verdict.json");
    const prompt = buildSessionPrompt(tail, s, verdictPath);
    writeFileSync27(join48(sessionTmp, "prompt.txt"), prompt);
    const gate = await runGateViaStdin({ agent: gateAgent, bin: gateBin, prompt, timeoutMs: GATE_TIMEOUT_MS });
    try {
      writeFileSync27(join48(sessionTmp, "gate-stdout.txt"), gate.stdout);
      if (gate.stderr)
        writeFileSync27(join48(sessionTmp, "gate-stderr.txt"), gate.stderr);
    } catch {
    }
    if (gate.errored) {
    ...
    const verdictText = existsSync37(verdictPath) ? readFileSync34(verdictPath, "utf-8") : gate.stdout;
...
function showStatus() {
  ...
  for (const f of files) {
    try {
      const s = JSON.parse(readFileSync35(join49(dir, f), "utf-8"));
...
function setInstall(loc2) {
  ...
  const path = loc2 === "global" ? join49(homedir25(), ".claude", "skills") : "<cwd>/.claude/skills";
...
function promoteSkill(name, cwd) {
  ...
  const projectPath = join49(cwd, ".claude", "skills", name);
  const globalPath = join49(homedir25(), ".claude", "skills", name);
  if (!existsSync38(join49(projectPath, "SKILL.md"))) {
    console.error(`Skill '${name}' not found at ${projectPath}/SKILL.md`);
    process.exit(1);
  }
  if (existsSync38(join49(globalPath, "SKILL.md"))) {
...
async function pullSkills(args) {
  ...
  const dest = toRaw === "global" ? join49(homedir25(), ".claude", "skills") : join49(process.cwd(), ".claude", "skills");
...
async function pushSkills(args) {
  ...
  const src = fromRaw === "global" ? join49(homedir25(), ".claude", "skills") : join49(process.cwd(), ".claude", "skills");
...
async function renderContextBlock(query, input, opts = {}) {
  ...
  try {
    ...
    if (tableExists && !tableExists(input.rulesTable)) {
      log9(`render-context-block: rules table "${input.rulesTable}" not present \u2014 skipping read`);
    } else {
      try {
        ...
        log9(`render-context-block: rules unavailable (continuing): ${rmsg}`);
      }
    }
    let goals = [];
    if (tableExists && !tableExists(input.goalsTable)) {
      log9(`render-context-block: goals table "${input.goalsTable}" not present \u2014 skipping read`);
    } else {
      try {
        ...
        log9(`render-context-block: goals unavailable (continuing): ${gmsg}`);
      }
    }
...
function projectNameFromCwd(cwd) {
  return basename3(cwd ?? "") || "unknown";
}

// dist/src/skillify/pending-memory-manifest.js
import { existsSync as existsSync39, mkdirSync as mkdirSync26, readFileSync as readFileSync36, renameSync as renameSync13, writeFileSync as writeFileSync28 } from "node:fs";
import { homedir as homedir26 } from "node:os";
import { dirname as dirname19, join as join50 } from "node:path";
var HIVEMIND_DIR3 = join50(homedir26(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join50(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join50(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join50(HIVEMIND_DIR3, "pending-memory.lock");
...
function writePendingMemoryManifest(m, path = PENDING_MEMORY_MANIFEST_PATH) {
  mkdirSync26(dirname19(path), { recursive: true });
  const tmp = `${path}.tmp`;
  writeFileSync28(tmp, JSON.stringify(m, null, 2));
...
import { existsSync as existsSync41, mkdirSync as mkdirSync29, readFileSync as readFileSync39, writeFileSync as writeFileSync30, unlinkSync as unlinkSync15 } from "node:fs";
import { join as join57 } from "node:path";

// dist/src/hooks/spawn-wiki-worker.js
import { fileURLToPath as fileURLToPath2 } from "node:url";
import { dirname as dirname21, join as join54 } from "node:path";
import { writeFileSync as writeFileSync29, mkdirSync as mkdirSync28 } from "node:fs";
import { homedir as homedir28, tmpdir as tmpdir2 } from "node:os";

// dist/src/utils/wiki-log.js
import { mkdirSync as mkdirSync27, appendFileSync as appendFileSync4 } from "node:fs";
import { join as join51 } from "node:path";
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join51(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
        mkdirSync27(hooksDir, { recursive: true });
        appendFileSync4(path, `[${utcTimestamp()}] ${msg}
`);
      } catch {
      }
    }
...
function resolveCliBin(cli, fallback) {
  ...
  try {
    const out = execFileSync6(isWin ? "where" : "which", [cli], { encoding: "utf-8" });
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join53(homedir27(), ".claude", "local", cli);
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir28();
var wikiLogger = makeWikiLogger(join54(HOME3, ".claude", "hooks"));
...
import { openSync as openSync3, closeSync as closeSync3, writeSync as writeSync2, unlinkSync as unlinkSync14, existsSync as existsSync40, readFileSync as readFileSync38 } from "node:fs";
import { homedir as homedir29 } from "node:os";
import { join as join55 } from "node:path";
var SHARED_DAEMON_PATH2 = join55(homedir29(), ".hivemind", "embed-deps", "embed-daemon.js");
var log8 = (m) => log2("embed-client", m);
function getUid() {
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync40(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync40(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    try {
      ...
      log8(`hello probe failed (inconclusive, will retry next connect): ${e instanceof Error ? e.message : String(e)}`);
      return false;
    }
    const hello = resp;
    ...
    if (!hello.daemonPath) {
      ...
      log8(`daemon does not implement hello (older protocol); recycling`);
      this.recycleDaemon(hello.pid);
      return true;
    }
    if (hello.daemonPath !== this.daemonEntry && !existsSync40(hello.daemonPath)) {
      _recycledStuckDaemon = true;
      log8(`daemon path no longer on disk \u2014 running=${hello.daemonPath} (gone) expected=${this.daemonEntry}; recycling`);
      this.recycleDaemon(hello.pid);
      return true;
    }
    this.helloVerified = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync38(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync40(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
    } else if (pid !== null) {
      log8(`recycle: socket gone, skipping SIGTERM on possibly-stale pid ${pid}`);
    }
    try {
      unlinkSync14(this.socketPath);
    } catch {
    }
    try {
      unlinkSync14(this.pidPath);
    } catch {
    }
  }
  /**
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync40(this.daemonEntry)) {
      log8(`daemonEntry not configured or missing: ${this.daemonEntry}`);
      try {
        closeSync3(fd);
        unlinkSync14(this.pidPath);
      } catch {
    ...
    try {
      ...
      child.unref();
      log8(`spawned daemon pid=${child.pid}`);
    } finally {
      closeSync3(fd);
    }
  }
  isPidFileStale() {
    try {
      const raw = readFileSync38(this.pidPath, "utf-8").trim();
...
function defaultResolveTransformers() {
  const sharedDir = join56(homedir30(), ".hivemind", "embed-deps");
  try {
...
function countLines(path) {
  try {
    const buf = readFileSync39(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
function runClaude2(claudeBin, prompt, timeoutMs) {
  ...
  return new Promise((resolve7) => {
    const child = spawn4(plan.file, plan.args, {
      stdio: plan.stdio,
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join57(stagingDir, `${key}.md`);
  const embeddingPath = join57(stagingDir, `${key}.embedding.json`);
  if (!existsSync41(input.jsonlPath)) {
    return { sessionId: key, ok: false, embedded: false, reason: "jsonl-missing" };
  }
  try {
    mkdirSync29(stagingDir, { recursive: true });
  } catch {
  ...
  if (existsSync41(summaryPath)) {
    try {
      unlinkSync15(summaryPath);
    } catch {
    }
  }
  ...
  const text = readFileSync39(summaryPath, "utf-8");
  if (!text.trim()) {
  ...
  if (!opts.skipEmbed) {
    try {
      ...
      if (vec) {
        writeFileSync30(embeddingPath, JSON.stringify(vec));
        embedded = true;
      }
    } catch {
    }
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync42, unlinkSync as unlinkSync16 } from "node:fs";
...
function releaseBackfillLock(lockPath2 = PENDING_MEMORY_LOCK_PATH) {
  if (process.env.HIVEMIND_BACKFILL_LOCK_OWNED !== "1")
    return;
  try {
    if (existsSync42(lockPath2))
      unlinkSync16(lockPath2);
  } catch {
  }
}
...
async function uploadSummary(query, params) {
  ...
  const sql = `INSERT INTO "${tableName}" (id, path, filename, summary, summary_embedding, author, mime_type, size_bytes, project, description, agent, plugin_version, creation_date, last_update_date) VALUES ('${randomUUID6()}', '${esc3(vpath)}', '${esc3(fname)}', E'${esc3(text)}', ${embSql}, '${esc3(userName)}', 'text/markdown', ${sizeBytes}, '${esc3(project)}', E'${esc3(desc)}', '${esc3(agent)}', '${esc3(pluginVersionForInsert)}', '${ts}', '${ts}')`;
  await query(sql);
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync40(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  const manifestPath3 = deps.manifestPath ?? PENDING_MEMORY_MANIFEST_PATH;
  const manifest = readPendingMemoryManifest(manifestPath3);
  ...
  for (const entry of pending) {
    if (!existsSync43(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync40(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
    ...
    try {
      ...
      markUploaded(entry.session_id, config.orgName, (/* @__PURE__ */ new Date()).toISOString(), manifestPath3);
      result.uploaded++;
    } catch {
      result.failed++;
    }
...
import { closeSync as closeSync5, existsSync as existsSync45, mkdirSync as mkdirSync31, openSync as openSync5, statSync as statSync9, unlinkSync as unlinkSync18 } from "node:fs";
...
import { closeSync as closeSync4, existsSync as existsSync44, mkdirSync as mkdirSync30, openSync as openSync4, readdirSync as readdirSync9, statSync as statSync8, unlinkSync as unlinkSync17 } from "node:fs";
import { homedir as homedir32 } from "node:os";
import { dirname as dirname22, join as join58 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var HOME4 = homedir32();
var HIVEMIND_DIR4 = join58(HOME4, ".claude", "hivemind");
var LOG_PATH = join58(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join58(HOME4, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname22(fileURLToPath3(import.meta.url));
    const cliPath = join58(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync44(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
function findHivemindLauncher() {
  ...
  try {
    ...
    const out = execFileSync7(lookup, ["hivemind"], {
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "ignore"]
    });
    const bin = out.trim();
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir33();
var HIVEMIND_DIR5 = join59(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join59(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync31(join59(HOME5, ".claude", "hooks"), { recursive: true });
  const out = openSync5(LOG_PATH2, "a");
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync45(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync45(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
    ...
    removeLock: () => {
      try {
        unlinkSync18(PENDING_MEMORY_LOCK_PATH);
        return true;
      } catch {
        return false;
      }
    ...
    acquireLock: () => {
      try {
        mkdirSync31(HIVEMIND_DIR5, { recursive: true });
        closeSync5(openSync5(PENDING_MEMORY_LOCK_PATH, "wx"));
        return true;
      } catch {
        return false;
      }
...
import { closeSync as closeSync6, existsSync as existsSync46, mkdirSync as mkdirSync32, openSync as openSync6, readFileSync as readFileSync41, realpathSync, unlinkSync as unlinkSync19, writeSync as writeSync3 } from "node:fs";
import { homedir as homedir34 } from "node:os";
import { dirname as dirname23, join as join60, sep as sep2 } from "node:path";
...
function defaultLockPath() {
  return join60(homedir34(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  installDir ??= dirname23(realArgv1);
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync46(`${gitDir}${sep2}.git`)) {
      return { kind: "local-dev", installDir };
    }
    const parent = dirname23(gitDir);
    if (parent === gitDir)
      break;
    gitDir = parent;
  }
...
function tryAcquireLock(path) {
  mkdirSync32(dirname23(path), { recursive: true, mode: 448 });
  const claim = () => {
  ...
  let holderPid = 0;
  try {
    holderPid = Number(readFileSync41(path, "utf-8").trim()) || 0;
  } catch {
    try {
      return claim();
    } catch {
...
async function runUpdate(opts = {}) {
  ...
  const spawn9 = opts.spawn ?? defaultSpawn;
  switch (detected.kind) {
    case "npm-global": {
      ...
      try {
        ...
        try {
          spawn9("npm", ["install", "-g", `${PKG_NAME}@${latest}`]);
        } catch (e) {
        ...
        try {
          spawn9("hivemind", ["install", "--skip-auth"]);
        } catch (e) {
...
import { existsSync as existsSync48, readFileSync as readFileSync42, readdirSync as readdirSync10, unlinkSync as unlinkSync20 } from "node:fs";
...
async function runAdvisor(manifestPath3 = LOCAL_MANIFEST_PATH) {
  ...
  if (!claudeBin || !existsSync47(claudeBin)) {
    return null;
  }
...
function claudeProjectsDir() {
  return join61(homedir35(), ".claude", "projects");
}
function manifestPath2() {
  return join61(homedir35(), ".claude", "hivemind", "local-mined.json");
}
...
function manifestIsTrulyEmpty() {
  ...
  if (!existsSync48(p))
    return false;
  try {
    const m = JSON.parse(readFileSync42(p, "utf-8"));
    return Array.isArray(m.entries) && m.entries.length === 0;
  } catch {
    return false;
  }
...
function hasLocalClaudeSessions() {
  ...
  if (!existsSync48(projectsDir))
    return false;
  let subdirs;
  try {
    subdirs = readdirSync10(projectsDir);
  } catch {
    return false;
  }
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync10(join61(projectsDir, sub));
    } catch {
      continue;
    }
...
function runInstallScan() {
  return new Promise((resolve7) => {
    ...
    if (!cliPath || !existsSync48(cliPath)) {
      resolve7({ insight: null, skillsCount: 0 });
      return;
    }
    const child = spawn8(process.execPath, [
      cliPath,
      "skillify",
      "mine-local",
      "--n",
