...
function runHostPrompt(spec, bin, prompt, timeoutMs = 12e4) {
  ...
  const out = execFileSync5(inv.file, inv.args, {
    ...inv.options,
    encoding: "utf-8",
    timeout: timeoutMs,
    env: { ...process.env, HIVEMIND_WIKI_WORKER: "1", HIVEMIND_CAPTURE: "false" }
...
import { mkdirSync as mkdirSync14, readFileSync as readFileSync23, writeFileSync as writeFileSync18, renameSync as renameSync8, existsSync as existsSync31 } from "node:fs";
import { homedir as homedir20 } from "node:os";
import { createHash } from "node:crypto";
import { dirname as dirname11, join as join35 } from "node:path";
function privateStoreRoot() {
  return process.env.HIVEMIND_DOCS_PRIVATE_DIR || join35(homedir20(), ".hivemind", "docs-private");
}
function storeFile(project, scope) {
  const key = createHash("sha256").update(`${project}\0${scope}`).digest("hex");
  return join35(privateStoreRoot(), `${key}.json`);
}
...
function writeMap(file, map2) {
  mkdirSync14(dirname11(file), { recursive: true });
...
async function runLocalWikiRefresh(args) {
  ...
  for (const group of selectWikiGroups(args.snap)) {
    ...
    const abs = join36(args.repoRoot, localFile);
    if (!existsSync32(abs)) {
...
import { existsSync as existsSync33, mkdirSync as mkdirSync15, readFileSync as readFileSync25, rmSync as rmSync8, writeFileSync as writeFileSync20 } from "node:fs";
import { dirname as dirname12, join as join37, resolve as resolve2, sep } from "node:path";
...
function readPullManifest(repoRoot) {
  try {
    const raw = JSON.parse(readFileSync25(join37(repoRoot, PULL_MANIFEST_DIR, PULL_MANIFEST_FILE), "utf-8"));
...
function writePullManifest(repoRoot, manifest) {
  const dir = join37(repoRoot, PULL_MANIFEST_DIR);
  mkdirSync15(dir, { recursive: true });
  writeFileSync20(join37(dir, PULL_MANIFEST_FILE), JSON.stringify(manifest, null, 2) + "\n");
}
function ensureGitignoreEntries(repoRoot) {
  const path = join37(repoRoot, ".gitignore");
  let current = "";
  try {
...
async function pullDocs(args) {
  ...
  for (const doc of latest.values()) {
    ...
    mkdirSync15(dirname12(abs), { recursive: true });
...
var SHARED_DAEMON_PATH2 = join38(homedir21(), ".hivemind", "embed-deps", "embed-daemon.js");
...
function defaultResolveTransformers() {
  const sharedDir = join39(homedir22(), ".hivemind", "embed-deps");
  try {
...
function resolveEmbedDaemonPath() {
  return join40(dirname13(fileURLToPath(import.meta.url)), "embeddings", "embed-daemon.js");
}
...
function gitHeadOf(cwd) {
  try {
    return execFileSync6("git", ["-C", cwd, "rev-parse", "HEAD"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim() || null;
  } catch {
    return null;
  }
}
...
wiki: ${planned.length - pending.length}/${planned.length} pages generated \u2014 pending: ${pending.map((g) => wikiDocId(g.key)).join(", ")}`);
  ...
  if (sub === "wiki-refresh") {
    ...
    const git = (gitArgs) => {
      try {
        return execFileSync6("git", ["-C", cwd, ...gitArgs], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
      } catch {
        return null;
      }
    };
  ...
  if (sub === "sync") {
    ...
    if (!loadCurrentSnapshot(cwd)) {
      ...
      const { runBuildCommand } = await import("./graph-OVYDN57Q.js");
...
import { existsSync as existsSync36, mkdirSync as mkdirSync16, readFileSync as readFileSync28, renameSync as renameSync9, writeFileSync as writeFileSync21 } from "node:fs";
import { homedir as homedir23 } from "node:os";
import { dirname as dirname14, join as join41 } from "node:path";
var HIVEMIND_DIR3 = join41(homedir23(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join41(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join41(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join41(HIVEMIND_DIR3, "pending-memory.lock");
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir24();
var wikiLogger = makeWikiLogger(join44(HOME3, ".claude", "hooks"));
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join45(stagingDir, `${key}.md`);
  const embeddingPath = join45(stagingDir, `${key}.embedding.json`);
...
import { closeSync as closeSync4, existsSync as existsSync41, mkdirSync as mkdirSync21, openSync as openSync4, statSync as statSync8, unlinkSync as unlinkSync14 } from "node:fs";
import { spawn as spawn7 } from "node:child_process";
import { homedir as homedir27 } from "node:os";
import { join as join47 } from "node:path";

// dist/src/skillify/spawn-mine-local-worker.js
import { execFileSync as execFileSync7, spawn as spawn6 } from "node:child_process";
import { closeSync as closeSync3, existsSync as existsSync40, mkdirSync as mkdirSync20, openSync as openSync3, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync13 } from "node:fs";
import { homedir as homedir26 } from "node:os";
import { dirname as dirname17, join as join46 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var HOME4 = homedir26();
var HIVEMIND_DIR4 = join46(HOME4, ".claude", "hivemind");
var LOG_PATH = join46(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join46(HOME4, ".claude", "projects");
var LOCK_STALE_MS = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname17(fileURLToPath3(import.meta.url));
    const cliPath = join46(thisDir, "..", "..", "bundle", "cli.js");
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync8(join46(CLAUDE_PROJECTS_DIR, sub));
    } catch {
      continue;
    }
...
function maybeAutoMineLocal(opts = {}) {
  ...
  try {
    mkdirSync20(join46(HOME4, ".claude", "hooks"), { recursive: true });
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir27();
var HIVEMIND_DIR5 = join47(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join47(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync21(join47(HOME5, ".claude", "hooks"), { recursive: true });
...
import { dirname as dirname18, join as join48, sep as sep2 } from "node:path";
...
function defaultLockPath() {
  return join48(homedir28(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  installDir ??= dirname18(realArgv1);
...
function tryAcquireLock(path) {
  mkdirSync22(dirname18(path), { recursive: true, mode: 448 });
  const claim = () => {
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-OVYDN57Q.js"));
    } catch (err) {
