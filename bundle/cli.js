...
function pushJsonlFile(out, agent, dir, fileName, inCwd) {
  if (!fileName.endsWith(".jsonl"))
    return;
  const fullPath = join43(dir, fileName);
  let stats;
  try {
...
function collectJsonlRecursive(out, agent, dir, inCwd) {
  ...
  for (const e of entries) {
    if (e.isDirectory())
      collectJsonlRecursive(out, agent, join43(dir, e.name), inCwd);
    else if (e.isFile())
...
function listLocalSessions(installs, cwd) {
  ...
  for (const install of installs) {
    ...
    for (const entry of top) {
      ...
      if (entry.isDirectory()) {
        collectJsonlRecursive(out, install.agent, join43(install.sessionRoot, entry.name), inCwd);
...
async function pullSkills(args) {
  ...
  const dest = toRaw === "global" ? join47(homedir25(), ".claude", "skills") : join47(process.cwd(), ".claude", "skills");
...
import { existsSync as existsSync37, mkdirSync as mkdirSync26, readFileSync as readFileSync34, renameSync as renameSync13, writeFileSync as writeFileSync27 } from "node:fs";
import { homedir as homedir26 } from "node:os";
import { dirname as dirname19, join as join48 } from "node:path";
var HIVEMIND_DIR3 = join48(homedir26(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join48(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join48(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join48(HIVEMIND_DIR3, "pending-memory.lock");
...
import { existsSync as existsSync39, mkdirSync as mkdirSync29, readFileSync as readFileSync37, writeFileSync as writeFileSync29, unlinkSync as unlinkSync14 } from "node:fs";
...
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join49(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
function resolveCliBin(cli, fallback) {
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join51(homedir27(), ".claude", "local", cli);
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir28();
var wikiLogger = makeWikiLogger(join52(HOME3, ".claude", "hooks"));
...
import { openSync as openSync3, closeSync as closeSync3, writeSync as writeSync2, unlinkSync as unlinkSync13, existsSync as existsSync38, readFileSync as readFileSync36 } from "node:fs";
import { homedir as homedir29 } from "node:os";
import { join as join53 } from "node:path";
var SHARED_DAEMON_PATH2 = join53(homedir29(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync38(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync38(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    if (hello.daemonPath !== this.daemonEntry && !existsSync38(hello.daemonPath)) {
      _recycledStuckDaemon = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (pid === null) {
      try {
        pid = Number.parseInt(readFileSync36(this.pidPath, "utf-8").trim(), 10);
      } catch {
      }
    }
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync38(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync38(this.daemonEntry)) {
  ...
  isPidFileStale() {
    try {
      const raw = readFileSync36(this.pidPath, "utf-8").trim();
...
function defaultResolveTransformers() {
  const sharedDir = join54(homedir30(), ".hivemind", "embed-deps");
  try {
...
function countLines(path) {
  try {
    const buf = readFileSync37(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join55(stagingDir, `${key}.md`);
  const embeddingPath = join55(stagingDir, `${key}.embedding.json`);
  if (!existsSync39(input.jsonlPath)) {
  ...
  const text = readFileSync37(summaryPath, "utf-8");
  if (!text.trim()) {
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync40, unlinkSync as unlinkSync15 } from "node:fs";
...
async function uploadSummary(query, params) {
  ...
  const existing = await query(`SELECT path FROM "${tableName}" WHERE path = '${esc2(vpath)}' LIMIT 1`);
  if (existing.length > 0) {
    const pluginVersionSet = pluginVersion === void 0 ? "" : `plugin_version = '${esc2(pluginVersion)}', `;
    const sql2 = `UPDATE "${tableName}" SET summary = E'${esc2(text)}', summary_embedding = ${embSql}, size_bytes = ${sizeBytes}, description = E'${esc2(desc)}', ` + pluginVersionSet + `last_update_date = '${ts}' WHERE path = '${esc2(vpath)}'`;
    await query(sql2);
    return { path: "update", sql: sql2, descLength: desc.length, summaryLength: text.length };
  }
  const pluginVersionForInsert = pluginVersion ?? "";
  const sql = `INSERT INTO "${tableName}" (id, path, filename, summary, summary_embedding, author, mime_type, size_bytes, project, description, agent, plugin_version, creation_date, last_update_date) VALUES ('${randomUUID5()}', '${esc2(vpath)}', '${esc2(fname)}', E'${esc2(text)}', ${embSql}, '${esc2(userName)}', 'text/markdown', ${sizeBytes}, '${esc2(project)}', E'${esc2(desc)}', '${esc2(agent)}', '${esc2(pluginVersionForInsert)}', '${ts}', '${ts}')`;
  await query(sql);
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync38(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync41(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync38(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
...
import { closeSync as closeSync5, existsSync as existsSync43, mkdirSync as mkdirSync31, openSync as openSync5, statSync as statSync9, unlinkSync as unlinkSync17 } from "node:fs";
...
import { closeSync as closeSync4, existsSync as existsSync42, mkdirSync as mkdirSync30, openSync as openSync4, readdirSync as readdirSync9, statSync as statSync8, unlinkSync as unlinkSync16 } from "node:fs";
import { homedir as homedir32 } from "node:os";
import { dirname as dirname22, join as join56 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
var HOME4 = homedir32();
var HIVEMIND_DIR4 = join56(HOME4, ".claude", "hivemind");
var LOG_PATH = join56(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join56(HOME4, ".claude", "projects");
var LOCK_STALE_MS2 = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname22(fileURLToPath3(import.meta.url));
    const cliPath = join56(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync42(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir33();
var HIVEMIND_DIR5 = join57(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join57(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync31(join57(HOME5, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync43(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync43(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
...
import { closeSync as closeSync6, existsSync as existsSync44, mkdirSync as mkdirSync32, openSync as openSync6, readFileSync as readFileSync39, realpathSync, unlinkSync as unlinkSync18, writeSync as writeSync3 } from "node:fs";
import { homedir as homedir34 } from "node:os";
import { dirname as dirname23, join as join58, sep as sep2 } from "node:path";
...
function defaultLockPath() {
  return join58(homedir34(), ".deeplake", "hivemind-update.lock");
}
function detectInstallKind(argv1) {
  ...
  for (let i = 0; i < 6; i++) {
    if (existsSync44(`${gitDir}${sep2}.git`)) {
...
function tryAcquireLock(path) {
  ...
  let holderPid = 0;
  try {
    holderPid = Number(readFileSync39(path, "utf-8").trim()) || 0;
  } catch {
    try {
      return claim();
    } catch {
...
import { existsSync as existsSync46, readFileSync as readFileSync40, readdirSync as readdirSync10, unlinkSync as unlinkSync19 } from "node:fs";
...
async function runAdvisor(manifestPath3 = LOCAL_MANIFEST_PATH) {
  ...
  if (!claudeBin || !existsSync45(claudeBin)) {
    return null;
  }
...
function claudeProjectsDir() {
  return join59(homedir35(), ".claude", "projects");
}
function manifestPath2() {
  return join59(homedir35(), ".claude", "hivemind", "local-mined.json");
}
...
function manifestIsTrulyEmpty() {
  ...
  if (!existsSync46(p))
    return false;
  try {
    const m = JSON.parse(readFileSync40(p, "utf-8"));
    return Array.isArray(m.entries) && m.entries.length === 0;
  } catch {
    return false;
  }
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync10(join59(projectsDir, sub));
    } catch {
      continue;
    }
...
function runInstallScan() {
  return new Promise((resolve7) => {
    ...
    if (!cliPath || !existsSync46(cliPath)) {
