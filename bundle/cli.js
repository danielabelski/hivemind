...
import { existsSync as existsSync4, copyFileSync, rmSync as rmSync2 } from "node:fs";
...
import { existsSync as existsSync5, lstatSync as lstatSync2, rmSync as rmSync3, unlinkSync as unlinkSync2 } from "node:fs";
...
import { existsSync as existsSync7, lstatSync as lstatSync3, writeFileSync as writeFileSync4, readFileSync as readFileSync4, rmSync as rmSync4, unlinkSync as unlinkSync3 } from "node:fs";
...
import { copyFileSync as copyFileSync3, chmodSync, existsSync as existsSync11, lstatSync as lstatSync4, readdirSync, readFileSync as readFileSync8, readlinkSync, rmSync as rmSync6, statSync, unlinkSync as unlinkSync5 } from "node:fs";
...
import { existsSync as existsSync10, mkdirSync, readFileSync as readFileSync7, renameSync as renameSync2, writeFileSync as writeFileSync7 } from "node:fs";
...
function isSymlinkToSharedDeps(linkPath, sharedNodeModules) {
  if (!existsSync11(linkPath))
    return false;
  try {
    if (!lstatSync4(linkPath).isSymbolicLink())
      return false;
...
function linkStateFor(install, sharedNodeModules = SHARED_NODE_MODULES) {
  ...
  try {
    if (lstatSync4(link).isSymbolicLink()) {
...
import { dirname as dirname5, join as join21, resolve } from "node:path";

// dist/src/dashboard/data.js
import { existsSync as existsSync17, readFileSync as readFileSync13, readdirSync as readdirSync3, statSync as statSync2 } from "node:fs";
...
function cacheFilePath() {
  return join14(homedir5(), ".deeplake", "hivemind-stats-cache.json");
}
...
function writeCache(scopeKey, data) {
  try {
    mkdirSync3(dirname2(cacheFilePath()), { recursive: true });
...
function statsFilePath() {
  return join15(homedir6(), ".deeplake", "usage-stats.jsonl");
}
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join15(homedir6(), ".claude", "skills");
  if (!existsSync14(dir))
    return 0;
...
import { readFileSync as readFileSync12, writeFileSync as writeFileSync10, writeSync, mkdirSync as mkdirSync5, renameSync as renameSync4, rmdirSync, existsSync as existsSync16, lstatSync as lstatSync5, unlinkSync as unlinkSync6, openSync, closeSync } from "node:fs";
...
function getStateDir() {
  ...
  return override && override.length > 0 ? override : join16(homedir7(), ".deeplake", "state", "skillify");
}
...
function migrateLegacyStateDir() {
  ...
  const legacy = join17(dirname4(current), "skilify");
...
function graphsRoot() {
  return process.env.HIVEMIND_GRAPHS_HOME ?? join19(homedir8(), ".hivemind", "graphs");
}
...
function resolveSnapshot(repoDir) {
  const snapshotsDir = join19(repoDir, "snapshots");
  if (!existsSync17(snapshotsDir))
    return null;
  let snapshotPath = null;
  const pointer = join19(repoDir, "latest-commit.txt");
...
async function loadDashboardData(opts = {}) {
  ...
  const repoDir = join19(opts.graphsHome ?? graphsRoot(), repoKey);
...
function defaultDashboardOutPath(repoKey) {
  return join21(homedir9(), ".hivemind", "dashboards", repoKey, "index.html");
}
...
`);
  ...
  try {
    mkdirSync6(dirname5(absOut), { recursive: true });
...
import { readdirSync as readdirSync7, existsSync as existsSync29, readFileSync as readFileSync22, mkdirSync as mkdirSync13, renameSync as renameSync7 } from "node:fs";
...
import { existsSync as existsSync22, readFileSync as readFileSync17, writeFileSync as writeFileSync15, mkdirSync as mkdirSync10, renameSync as renameSync6, lstatSync as lstatSync7, readlinkSync as readlinkSync2, symlinkSync, unlinkSync as unlinkSync8 } from "node:fs";
import { homedir as homedir12 } from "node:os";
import { dirname as dirname7, join as join26 } from "node:path";

// dist/src/skillify/skill-writer.js
import { existsSync as existsSync19, mkdirSync as mkdirSync8, readFileSync as readFileSync15, readdirSync as readdirSync4, statSync as statSync4, writeFileSync as writeFileSync13 } from "node:fs";
...
function listSkills(skillsRoot) {
  ...
  for (const name of readdirSync4(skillsRoot)) {
    ...
    if (existsSync19(skillFile) && statSync4(skillFile).isFile()) {
...
function resolveSkillsRoot(install, cwd) {
  if (install === "global") {
    return join23(homedir10(), ".claude", "skills");
  }
  return join23(cwd, ".claude", "skills");
}

// dist/src/skillify/manifest.js
import { existsSync as existsSync20, lstatSync as lstatSync6, mkdirSync as mkdirSync9, readFileSync as readFileSync16, renameSync as renameSync5, unlinkSync as unlinkSync7, writeFileSync as writeFileSync14 } from "node:fs";
...
function resolvePullDestination(install, cwd) {
  if (install === "global")
    return join26(homedir12(), ".claude", "skills");
  if (!cwd)
...
async function runPull(opts) {
  ...
  for (const row of latest) {
    ...
    if (action === "wrote") {
      ...
      writeFileSync15(skillFile, renderSkillFile(row));
...
import { existsSync as existsSync24, readdirSync as readdirSync5, rmSync as rmSync7, statSync as statSync5 } from "node:fs";
import { homedir as homedir13 } from "node:os";
import { join as join28 } from "node:path";
function resolveUnpullRoot(install, cwd) {
  if (install === "global")
    return join28(homedir13(), ".claude", "skills");
  if (!cwd)
...
import { basename, dirname as dirname9, join as join32 } from "node:path";

// dist/src/skillify/local-source.js
import { readdirSync as readdirSync6, readFileSync as readFileSync19, existsSync as existsSync25, statSync as statSync6 } from "node:fs";
...
var LOCAL_MANIFEST_PATH = join31(homedir16(), ".claude", "hivemind", "local-mined.json");
var LOCAL_MINE_LOCK_PATH = join31(homedir16(), ".claude", "hivemind", "local-mined.lock");
...
async function runMineLocalImpl(args) {
  ...
  const tmpDir = join32(homedir17(), ".claude", "hivemind", `mine-local-${Date.now()}`);
  ...
  const results = await parallelMap(picked, GATE_CONCURRENCY, async (s) => {
    ...
    const verdictText = existsSync28(verdictPath) ? readFileSync21(verdictPath, "utf-8") : gate.stdout;
  ...
  for (const { skill, session } of flat) {
    ...
    try {
      ...
      const canonicalDir = dirname9(result.path);
...
function setInstall(loc) {
  ...
  const path = loc === "global" ? join33(homedir18(), ".claude", "skills") : "<cwd>/.claude/skills";
...
function promoteSkill(name, cwd) {
  ...
  const globalPath = join33(homedir18(), ".claude", "skills", name);
  if (!existsSync29(join33(projectPath, "SKILL.md"))) {
    console.error(`Skill '${name}' not found at ${projectPath}/SKILL.md`);
    process.exit(1);
  }
  if (existsSync29(join33(globalPath, "SKILL.md"))) {
  ...
  mkdirSync13(dirname10(globalPath), { recursive: true });
...
async function pullSkills(args) {
  ...
  const dest = toRaw === "global" ? join33(homedir18(), ".claude", "skills") : join33(process.cwd(), ".claude", "skills");
...
async function pushSkills(args) {
  ...
  const src = fromRaw === "global" ? join33(homedir18(), ".claude", "skills") : join33(process.cwd(), ".claude", "skills");
...
import { existsSync as existsSync30, mkdirSync as mkdirSync14, readFileSync as readFileSync23, renameSync as renameSync8, writeFileSync as writeFileSync18 } from "node:fs";
import { homedir as homedir19 } from "node:os";
import { dirname as dirname11, join as join34 } from "node:path";
var HIVEMIND_DIR3 = join34(homedir19(), ".claude", "hivemind");
var PENDING_MEMORY_DIR = join34(HIVEMIND_DIR3, "pending-memory");
var PENDING_MEMORY_MANIFEST_PATH = join34(HIVEMIND_DIR3, "pending-memory.json");
var PENDING_MEMORY_LOCK_PATH = join34(HIVEMIND_DIR3, "pending-memory.lock");
...
function writePendingMemoryManifest(m, path = PENDING_MEMORY_MANIFEST_PATH) {
  ...
  const tmp = `${path}.tmp`;
  writeFileSync18(tmp, JSON.stringify(m, null, 2));
...
import { existsSync as existsSync32, mkdirSync as mkdirSync17, readFileSync as readFileSync26, writeFileSync as writeFileSync20, unlinkSync as unlinkSync11 } from "node:fs";
import { join as join41 } from "node:path";

// dist/src/hooks/spawn-wiki-worker.js
import { fileURLToPath } from "node:url";
import { dirname as dirname13, join as join38 } from "node:path";
import { writeFileSync as writeFileSync19, mkdirSync as mkdirSync16 } from "node:fs";
import { homedir as homedir21, tmpdir } from "node:os";

// dist/src/utils/wiki-log.js
import { mkdirSync as mkdirSync15, appendFileSync as appendFileSync2 } from "node:fs";
import { join as join35 } from "node:path";
function makeWikiLogger(hooksDir, filename = "deeplake-wiki.log") {
  const path = join35(hooksDir, filename);
  return {
    path,
    log(msg) {
      try {
...
function resolveCliBin(cli, fallback) {
  ...
  try {
    const out = execFileSync4(isWin ? "where" : "which", [cli], { encoding: "utf-8" });
  ...
  if (fallback !== void 0)
    return fallback;
  const local = join37(homedir20(), ".claude", "local", cli);
...
// dist/src/hooks/spawn-wiki-worker.js
var HOME3 = homedir21();
var wikiLogger = makeWikiLogger(join38(HOME3, ".claude", "hooks"));
...
import { openSync as openSync2, closeSync as closeSync2, writeSync as writeSync2, unlinkSync as unlinkSync10, existsSync as existsSync31, readFileSync as readFileSync25 } from "node:fs";
import { homedir as homedir22 } from "node:os";
import { join as join39 } from "node:path";
var SHARED_DAEMON_PATH2 = join39(homedir22(), ".hivemind", "embed-deps", "embed-daemon.js");
...
var EmbedClient = class {
  ...
  constructor(opts = {}) {
    ...
    this.daemonEntry = opts.daemonEntry ?? process.env.HIVEMIND_EMBED_DAEMON ?? (existsSync31(SHARED_DAEMON_PATH2) ? SHARED_DAEMON_PATH2 : void 0);
  ...
  async waitForDaemonReady() {
    ...
    while (Date.now() < deadline) {
      if (existsSync31(this.socketPath))
        return;
  ...
  async verifyDaemonOnce(sock) {
    ...
    if (hello.daemonPath !== this.daemonEntry && !existsSync31(hello.daemonPath)) {
      _recycledStuckDaemon = true;
  ...
  recycleDaemon(reportedPid) {
    ...
    if (Number.isFinite(pid) && pid !== null && pid > 0 && existsSync31(this.socketPath)) {
      try {
        process.kill(pid, "SIGTERM");
      } catch {
      }
  ...
  trySpawnDaemon() {
    ...
    if (!this.daemonEntry || !existsSync31(this.daemonEntry)) {
...
function defaultResolveTransformers() {
  const sharedDir = join40(homedir23(), ".hivemind", "embed-deps");
  try {
...
function countLines(path) {
  try {
    const buf = readFileSync26(path, "utf-8");
    if (!buf)
      return 0;
    return buf.endsWith("\n") ? buf.split("\n").length - 1 : buf.split("\n").length;
  } catch {
...
async function stageSession(input, opts) {
  ...
  const summaryPath = join41(stagingDir, `${key}.md`);
  const embeddingPath = join41(stagingDir, `${key}.embedding.json`);
  if (!existsSync32(input.jsonlPath)) {
    return { sessionId: key, ok: false, embedded: false, reason: "jsonl-missing" };
  }
  try {
    mkdirSync17(stagingDir, { recursive: true });
  } catch {
  ...
  const text = readFileSync26(summaryPath, "utf-8");
  if (!text.trim()) {
  ...
  if (!opts.skipEmbed) {
    try {
      ...
      if (vec) {
        writeFileSync20(embeddingPath, JSON.stringify(vec));
        embedded = true;
      }
    } catch {
    }
...
// dist/src/commands/backfill-memory.js
import { existsSync as existsSync33, unlinkSync as unlinkSync12 } from "node:fs";
...
function loadEmbedding(entry) {
  if (!entry.embedded || !entry.embedding_path)
    return null;
  try {
    const v = JSON.parse(readFileSync27(entry.embedding_path, "utf-8"));
...
async function runFlushMemory(deps = defaultDeps()) {
  ...
  for (const entry of pending) {
    if (!existsSync34(entry.summary_path)) {
      result.failed++;
      continue;
    }
    let text;
    try {
      text = readFileSync27(entry.summary_path, "utf-8");
    } catch {
      result.failed++;
      continue;
    }
...
import { closeSync as closeSync4, existsSync as existsSync36, mkdirSync as mkdirSync19, openSync as openSync4, statSync as statSync8, unlinkSync as unlinkSync14 } from "node:fs";
import { spawn as spawn6 } from "node:child_process";
import { homedir as homedir26 } from "node:os";
import { join as join43 } from "node:path";

// dist/src/skillify/spawn-mine-local-worker.js
import { execFileSync as execFileSync5, spawn as spawn5 } from "node:child_process";
import { closeSync as closeSync3, existsSync as existsSync35, mkdirSync as mkdirSync18, openSync as openSync3, readdirSync as readdirSync8, statSync as statSync7, unlinkSync as unlinkSync13 } from "node:fs";
import { homedir as homedir25 } from "node:os";
import { dirname as dirname14, join as join42 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
var HOME4 = homedir25();
var HIVEMIND_DIR4 = join42(HOME4, ".claude", "hivemind");
var LOG_PATH = join42(HOME4, ".claude", "hooks", "mine-local.log");
var CLAUDE_PROJECTS_DIR = join42(HOME4, ".claude", "projects");
var LOCK_STALE_MS = 15 * 60 * 1e3;
function findBundledCliPath() {
  try {
    const thisDir = dirname14(fileURLToPath2(import.meta.url));
    const cliPath = join42(thisDir, "..", "..", "bundle", "cli.js");
    return existsSync35(cliPath) ? cliPath : null;
  } catch {
    return null;
  }
}
...
// dist/src/skillify/spawn-backfill-memory-worker.js
var HOME5 = homedir26();
var HIVEMIND_DIR5 = join43(HOME5, ".claude", "hivemind");
var LOG_PATH2 = join43(HOME5, ".claude", "hooks", "backfill-memory.log");
function realSpawn() {
  const launcher = findHivemindLauncher();
  if (!launcher)
    return false;
  mkdirSync19(join43(HOME5, ".claude", "hooks"), { recursive: true });
...
function maybeAutoBackfillMemory() {
  return runBackfillGuards({
    manifestExists: () => existsSync36(PENDING_MEMORY_MANIFEST_PATH),
    lockExists: () => existsSync36(PENDING_MEMORY_LOCK_PATH),
    lockAgeMs: () => {
      try {
    ...
    acquireLock: () => {
      try {
        mkdirSync19(HIVEMIND_DIR5, { recursive: true });
...
import { closeSync as closeSync5, existsSync as existsSync37, mkdirSync as mkdirSync20, openSync as openSync5, readFileSync as readFileSync28, realpathSync, unlinkSync as unlinkSync15, writeSync as writeSync3 } from "node:fs";
import { homedir as homedir27 } from "node:os";
import { dirname as dirname15, join as join44, sep } from "node:path";
...
function defaultLockPath() {
  return join44(homedir27(), ".deeplake", "hivemind-update.lock");
}
...
import { existsSync as existsSync39, readFileSync as readFileSync29, readdirSync as readdirSync9, unlinkSync as unlinkSync16 } from "node:fs";
...
function claudeProjectsDir() {
  return join45(homedir28(), ".claude", "projects");
}
function manifestPath2() {
  return join45(homedir28(), ".claude", "hivemind", "local-mined.json");
}
...
function manifestIsTrulyEmpty() {
  ...
  if (!existsSync39(p))
    return false;
  try {
    const m = JSON.parse(readFileSync29(p, "utf-8"));
    return Array.isArray(m.entries) && m.entries.length === 0;
  } catch {
    return false;
  }
...
function hasLocalClaudeSessions() {
  ...
  for (const sub of subdirs) {
    let files;
    try {
      files = readdirSync9(join45(projectsDir, sub));
    } catch {
      continue;
    }
...
function runInstallScan() {
  return new Promise((resolve2) => {
    ...
    if (!cliPath || !existsSync39(cliPath)) {
      resolve2({ insight: null, skillsCount: 0 });
      return;
    }
...
async function main() {
  ...
  if (cmd === "graph") {
    let runGraphCommand;
    try {
      ({ runGraphCommand } = await import("./graph-RKG6TGEA.js"));
    } catch (err) {
