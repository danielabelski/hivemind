 ...
 *
 * Invoked by the capture hook as: node skilify-worker.js <config.json>
 */
...
function wlog(msg: string): void {
  try {
    appendFileSync(cfg.skilifyLog, `[${utcTimestamp()}] skilify-worker(${cfg.projectKey}): ${msg}\n`);
  } catch { /* ignore */ }
}
