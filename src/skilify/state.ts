/**
 * Per-project state for the skilify worker.
 *
 * File: ~/.deeplake/state/skilify/<projectKey>.json
 *   {
...
const dlog = (msg: string) => _log("skilify-state", msg);

export interface SkilifyState {
  project: string;
  projectKey: string;
  counter: number;
...
export function readState(projectKey: string): SkilifyState | null {
  const p = statePath(projectKey);
  if (!existsSync(p)) return null;
  try {
    return JSON.parse(readFileSync(p, "utf-8")) as SkilifyState;
  } catch {
    return null;
  }
}

export function writeState(projectKey: string, state: SkilifyState): void {
